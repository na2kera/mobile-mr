...
// ================= 3. game =================
{
  ...
  const seqBeforeReset = g.snapshot(1100).seq;
  const reset = g.resetPractice(1150);
  const resetSnap = g.snapshot(1150, true);
  check("練習中の resetPractice で格子・飛行中の弾・インク残量が初期化される", reset[0]?.kind === "reset" && g.phase === "practice" && g.scores().p1 === 0 && resetSnap.shots.length === 0 && resetSnap.ink.p1 === 1 && resetSnap.seq > seqBeforeReset && Object.values(resetSnap.grids).every((enc) => /^0*$/.test(enc)));
...
try {
  ...
  b.send({ type: "reset" });
  const rejPhoneReset = await b.waitFor((m) => m.type === "rejected" && /overview/.test(m.reason));
  check("スマホからの reset は rejected: not overview", rejPhoneReset !== null);
  ov.send({ type: "reset" });
  const resetState = await a.waitFor((m) => m.type === "state" && m.state.event?.kind === "reset");
  const resetOverview = await ov.waitFor((m) => m.type === "state" && m.state.event?.kind === "reset");
  check("俯瞰画面の reset で全員に格子付き state が届き、練習の塗り・弾・インク残量が初期化される", resetState && resetOverview && resetState.state.grids && (resetState.state.scores.p2 ?? 0) === 0 && resetState.state.shots.length === 0 && resetState.state.ink.p1 === 1 && resetState.state.ink.p2 === 1 && resetState.state.phase === "practice");
