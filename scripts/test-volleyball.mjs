...
// ================= 1. sim =================
{
  ...
  const nearNet = aimPoint([0, 0.9, 0.1], "A", DEFAULT_COURT);
  check("aimPoint: ネット際に立っても狙い点は自陣側（ネットから 0.25m 以上）", nearNet[2] >= 0.25);
  const nearNetB = aimPoint([0, 0.9, -0.05], "B", DEFAULT_COURT);
  check("aimPoint: B 側でも同様（負側）", nearNetB[2] <= -0.25);
...

{
  ...
  // A 側（Z>0）で追跡開始 → A に割当、B は bot
  game.updatePose("p1", [0, 0.9, 1.6], true, now);
  check("最初の追跡姿勢で Z の符号からサイド割当", game.state.sides.A === "p1" && game.state.sides.B === null);
...
// ネット高さの自動調整（頭の高さ - 0.35）
{
  ...
  let now = 0;
  game.join("p1", now);
  game.updatePose("p1", [0, 1.6, 1.6], true, now); // 床のマーカー + 立位
  now = runFor(game, 2000, now);
  ...
  fixed.join("p1", 0);
  fixed.updatePose("p1", [0, 1.6, 1.6], true, 0);
  runFor(fixed, 2000, 0);
...
function buildQuery(room, { v = 1, markerId = 0, markerMm = 100, netTop = "auto" } = {}) {
  return new URLSearchParams({ room, v: String(v), markerId: String(markerId), markerMm: String(markerMm), netTop: String(netTop) }).toString();
}
...
try {
  ...
  check("ラリー中は state が 20Hz 前後で届く（0.5 秒で 5 通以上）", countAfter - countBefore >= 5, `${countAfter - countBefore} msgs`);

  // ---- hit: 受理されたら即 state（event=hit, by=自分） ----
  // A の頭の前（aimPoint）にボールが来るのを待つ。相手（B）の姿勢を送り続けて stale にしない
  ...
  // 遠い hit は無視される（state の event に現れない）
