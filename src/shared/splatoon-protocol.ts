 ...
 * v11: 結果表示は時間では終わらず（phaseEndsAt は null）、俯瞰画面の dismiss（結果を閉じる）で練習に戻る（issue #45）
 */
export const SPLATOON_PROTOCOL_VERSION = 11;
...
export type ClientMessage =
  ...
  | { type: "reset" }
  /** 結果を閉じて練習に戻る（俯瞰画面だけが送れる。結果表示中だけ受け付ける。結果は時間では消えない。issue #45） */
  | { type: "dismiss" }
