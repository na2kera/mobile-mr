...
/**
 * 俯瞰画面（マスター）から変えられるフィールドの寸法（幅・高さ・奥行き [m]）。
 * URL クエリではなくサーバーの状態として持ち、俯瞰画面の「反映」で room 全員に配る
 * （マーカーの高さ floorDrop は現実の採寸なので URL のまま）
 */
export type FieldSize = Pick<FieldConfig, "wallW" | "wallH" | "floorDepth">;
export const FIELD_SIZE_KEYS = ["wallW", "wallH", "floorDepth"] as const;
