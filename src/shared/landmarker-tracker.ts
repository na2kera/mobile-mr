// MediaPipe Tasks Vision の *Landmarker（Hand / Pose）を <video> に対して回す共通部分。
// Phase 5 の hand-tracker.ts にあったものを、Phase 9 で PoseLandmarker にも同じ手順が要ることが
// 分かった時点で抽出した（hand-tracker.ts / pose-tracker.ts はタスク固有のオプションを足す薄い皮）。
// 段階1（既存ライブラリで作りきる）なので、MediaPipe をそのまま使う。ここで吸収しているのは次の3点だけ:
...
/** MediaPipe の WasmFileset のうち使う部分（ローダーと wasm の URL） */
export type WasmFileset = { wasmLoaderPath: string; wasmBinaryPath: string };

/** MediaPipe の各 Landmarker に共通の、ここで使うメソッドだけ */
export type VideoLandmarker<R> = {
  detectForVideo(source: HTMLVideoElement | HTMLCanvasElement, timestampMs: number): R;
  close(): void;
};

export type LandmarkerBaseOptions = {
  modelAssetBuffer: Uint8Array;
  delegate: Delegate;
};

export type LandmarkerTrackerOptions<R> = {
  /** エラーメッセージ用の名前（"HandLandmarker" など） */
  label: string;
  /** "auto" は GPU → 失敗したら CPU */
  delegate: Delegate | "auto";
  /** モデル（*.task）の取得先候補。先頭から順に試す */
  modelUrls: string[];
  /**
   * 推論に渡す画像の長辺の上限 [px]。0 なら video をそのまま渡す。
   * モデルの入力は 192〜256px 程度なので、被写体が画面の大部分を占めるなら 640 で十分
   */
  inputMaxSide: number;
  /**
  ...
  /** wasm・モデル・デリゲートを受け取り、タスク固有のオプションを足して Landmarker を作る */
  create(fileset: WasmFileset, baseOptions: LandmarkerBaseOptions, canvas?: HTMLCanvasElement): Promise<VideoLandmarker<R>>;
};

export type LandmarkerTracker<R> = {
  /** 実際に初期化できたデリゲート */
 ...
 */
export async function createLandmarkerTracker<R>(
  opts: LandmarkerTrackerOptions<R>,
  onProgress: (step: string) => void,
): Promise<LandmarkerTracker<R>> {
  ...
  const fileset: WasmFileset = { wasmLoaderPath, wasmBinaryPath };
  const delegates: Delegate[] =
    opts.delegate === "auto" ? ["GPU", "CPU"] : [opts.delegate];
  let landmarker: VideoLandmarker<R> | null = null;
  let delegate: Delegate = delegates[0];
  const errors: string[] = [];
  for (const d of delegates) {
    onProgress(`init-${d.toLowerCase()}`);
    try {
      landmarker = await opts.create(
        fileset,
        { modelAssetBuffer: new Uint8Array(buffer), delegate: d },
        opts.canvas,
      );
      delegate = d;
      break;
  ...
  if (!landmarker) {
    throw new Error(`${opts.label} を初期化できません: ${errors.join(" / ")}`);
  }
...
async function fetchFirst(
  ...
  for (const url of urls) {
    ...
    try {
      ...
      // 手 7.8MB / 姿勢（lite）5.8MB のはず。開発サーバーが SPA フォールバックで index.html を
      // 返した場合等、極端に小さいものはモデルではないので弾く
