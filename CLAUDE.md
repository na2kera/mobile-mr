...
8. ~~Phase 8: MRスプラトゥーン~~ — 壁マーカー 1 枚で四方の壁 + 床の箱型コート（3.0×2.5m）を作り、パーの間「目 → 手」の向きへインクを連射（タンク 50 発・撃つのをやめると回復、グーで速く回復）、参加順に 8 色の個人戦。サーバー権威の格子で塗りと得点を共有（完了: `demos/08-splatoon/`、`server/splatoon.ts`。**iPhone 実機で確認済み** 2026-09-02。PR #15）
   ...
   - マルチマーカー（issue #30）: 床・左右・背面の壁の追加マーカーの配置（面 + 原点からの位置）をサーバーの状態にして俯瞰画面から配り、どれが見えてもアンカー（原点の姿勢）に直す（`src/shared/marker-layout.ts`、`marker-anchor.ts` の `extraMarkers`。アンカーは 1 つのまま）。フェイクカメラは field 座標系のマーカーをピンホール投影する 3D 版（`src/shared/fake-markers.ts`）。印刷は `markers.html`。プロトコル v9。PC 確認済み・**実機未確認**。追加マーカーから出した原点が POSIT の表裏 2 解の取り違えで 1〜2m 飛ぶ問題（issue #55）は、重力（ワールド +Y）で観測の傾きを直す `marker-anchor.ts` の `level`（`?maxTilt=`）で対処。PC 確認済み・**実機未確認**
