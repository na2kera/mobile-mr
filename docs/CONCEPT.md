...
Phase 5（Hand Tracking）まで完了、Phase 6（MRバレーボール）は実装済みで実機確認待ち。技術スタックは Vite / TypeScript / Three.js。
