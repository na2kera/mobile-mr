...
- **どう対処したか**: 組にするのは「マーカーの採用直後」かつ「直近の補正量 Δ が小さい（アンカーが追いついている）」フレームだけにした（`?slamPairMaxCorr=0.08`）。回転は位置の軌道からではなく各組の向きの差の平均で出す（ほぼ直線の軌道では位置から回転が決まらない）。確認では視軸から 5° 以内・0.5〜0.8m の位置で較正し、基線の閾値を下げた（`slamMinBaseline=0.06`）。鏡像解そのものは未解決（08 と同じ）。HUD に `off=`（マーカーと SLAM を写した位置の差）と `rotErr=`（SLAM の向きとジャイロの差）を出して実機で壊れた組に気付けるようにした。**実機は未確認**
