...
- **何が苦しかったか**: 塗りは canvas に蓄積していく（消せない）前提で、そこに「150ms で広がる」「壁で 1.2 秒垂れる」「塗り替えで一瞬白く光る」を足すと、途中の大きさで描いた滴の跡が点線として残る・フラッシュの白を消すには完成形を上から描き直す、といった「蓄積 canvas の上でのアニメ」特有の工夫が要った。着弾ごとに `needsUpdate` で全面転送するので、実機（384px/m × 5 面）での負荷は未確認
- **どう対処したか**: 広がり中は本体だけ描き、完成フレームで滴込みの形を上書き。フラッシュは本体のパスを白で塗り、最後にフラッシュ無しの完成形で確定。垂れは前フレームの終端から差分だけ描く。塗り替えの検出は着弾点の 1px を `getImageData` で読む
