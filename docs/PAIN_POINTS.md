...
- **どう対処したか**: 寸法を Room 設定（接続クエリ）から外してサーバーの状態（`game.config`）に移し、俯瞰画面だけが送れる `field` メッセージで変える（プロトコル v7。練習中か結果表示中だけ。格子はセル数が変わるので作り直し = 塗りは消える）。サーバーは `field`（config + 格子付き state）を全員に配り、クライアントは `applyFieldConfig` で形に効く値の差分を見て壁・床・塗り層・スコアボードを作り直す。寸法の範囲とセル数の上限（`validateFieldSize`）はサーバーと俯瞰画面の入力欄で共有。マーカーの高さ（`floorDrop`）・重力・時間は現実の採寸や飛行に効くので URL のまま（一致検証も残す）
