...
## [2026-09-11] Phase 8 / 08-splatoon: 平面マーカーの姿勢の「表裏 2 解」は原点マーカーでは無害だが、追加マーカーでは原点が 1〜2m 飛ぶ（issue #55）

- **何が苦しかったか**: マルチマーカー（issue #30）で床や横の壁のマーカーを見ているとき「補正されている感がない」（issue #55）。ロジック自体（anchor→world = marker→world × (marker→anchor)⁻¹）は正しく、PC のヘッドレス確認（合成カメラ・床マーカーを 0.5m の至近から）は通っていた。原因は幾何の違い: 平面 4 点の POSIT（js-aruco2 posit1.js）は「面の傾きが表か裏か」の 2 解を返し、マーカーが小さく映る（100mm を 2m から = 30px 前後）と 2 解の再投影誤差はほぼ同じで、best の選択は事実上ランダム（Node の実験で 2.2m 正対の原点マーカーは 65% が逆）。**原点マーカーではマーカー自身が原点なので取り違えても位置は 3〜6cm しかずれず、単一マーカーの Phase 3〜8 で気づかなかった**。追加マーカーでは「マーカー → 原点」の腕の長さ（1〜2m）で傾きの誤りが増幅され、立って床マーカーを見下ろす幾何で原点が p90 1.9m 飛ぶ（数フレームに 1 回）。lerp で平均されて意味のない位置に落ち着くので「補正されていない」ように見える。合成カメラは角が正確でこの取り違えが起きず、実機でしか出ない
- **どう対処したか**: DeviceOrientation 由来のカメラ姿勢はワールドの +Y が鉛直上なので、「マーカーは水平 / 鉛直に貼ってある」前提で観測した姿勢の傾きを重力に合わせて直す（`levelPose`: マーカーの中心まわりの最短回転。位置は信用し傾きだけ直す）。POSIT の 2 解は `alternative` として検出器から返し、傾きの小さい方を採り、傾きが上限（`?maxTilt=30`）を超える観測は捨てる。Node の実験（同じ POSIT + 画素ノイズ 0.5px）で床マーカーの原点誤差 p90 1.90m → 0.05m、IMU の重力誤差 3° を入れても 0.10m。HUD の `marker=` に `tilt=`（直す前の傾き）と `Δ=`（その観測がアンカーを動かした距離 = 補正量）を出して「本当に補正しているか」を実機で見られるようにした。ハマり: PC の合成カメラは映像だけが `fakePitch` で見下ろし、仮想カメラ（OrbitControls）は水平のままなので「ワールド +Y = 映像の上」が成り立たず全観測が棄却された → `level.worldUp` を関数にして合成カメラでは「映像の中の上」を仮想カメラ経由でワールドに直して渡す。06〜07・09（`level` 省略）は従来どおり。**実機は未確認**（IMU の重力の精度、正対した壁マーカーのヨー誤差 × 腕の長さ = 20〜40cm は残る）
- **SDK ならどう解決するか（案）**: 単一マーカーの姿勢推定を「マーカーの拘束（水平 / 鉛直・既知の配置）+ IMU の重力」と一緒に解く層を `@mobile-mr/spatial` が持つ（POSIT の 2 解の取り違え・ヨーの弱さは既知の性質なので、生の姿勢をアプリに渡さない）。合成カメラは「仮想カメラの姿勢 = 映像の姿勢」を保証し、実機でしか出ない幾何（遠い・斜め・小さい）をヘッドレスで再現できるようにする。実機で出た誤差を Node で再現する道具（POSIT + 投影 + ノイズ）は今回のテスト（`scripts/test-splatoon.mjs` の issue #55）の形で SDK のテスト資産にする
- **関連**: `src/shared/marker-layout.ts` の `levelPose`、`src/shared/marker-anchor.ts` の `level` / `anchorFromSolution`、`src/shared/marker-detector.ts` の `alternative`、`demos/08-splatoon/main.ts` の `worldUpForLeveling`、[issue #55](https://github.com/na2kera/mobile-mr/issues/55)、PAIN_POINTS「[2026-08-17] js-aruco2 POSIT の誤差 API に無効値の罠が2つある」
