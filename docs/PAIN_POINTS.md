...
## [2026-09-03] 全デモ共通 / 08-splatoon・09-person-id: スマホがかなり熱くなるが、熱の状態を知る API がなく、負荷をまとめて下げる場所もない

- **何が苦しかったか**: 08 / 09 をスマホ + ゴーグルで動かすと本体がかなり熱くなる。内訳は (1) ステレオ描画の fill 量: `setPixelRatio(Math.min(devicePixelRatio, 2))` で iPhone は 2 倍解像度を左右 2 回、全画面のパススルー背景ごと毎フレーム描く。フレームレートの上限もなく ProMotion 端末では 120fps まで回る (2) カメラ 1280×720 の常時取得と、毎フレームの video → テクスチャ転送 (3) MediaPipe の推論を映像フレームが来るたび（約 30fps）に回す。08 の手は `handDetW=0` で 720p をそのまま GPU デリゲートに渡している (4) ArUco 検出を 100ms ごとに 960px で `getImageData` → CPU（純粋 JS の js-aruco2）。この 4 つがデモごとに独立して「毎フレーム全力」で動いていて、まとめて調整する場所がない。言語や実装を変えても効かない: 重い処理（描画・推論・カメラ）は既に GPU / WebAssembly / OS 側で動いていて、JS が支配的なのは ArUco だけ。専用機（Quest 等）はファン・低解像度の白黒トラッキングカメラ・専用 DSP・カメラから合成器への直結パス・持続負荷前提のクロック制御で同じ問題を避けているが、スマホはファンなしでゴーグルに密閉され、SoC はバースト向けなので数分で熱制限に入る（Cardboard / Daydream の頃からの定番の問題に、カメラ常時取得と推論を上乗せしている）。実害は熱そのものより熱制限で、フレームレートが落ちて頭追従が遅れ、酔いにつながる。ブラウザには温度や熱制限を読む API がないので、状態を直接知れない
- **どう対処したか**: 未解決。URL パラメータで絞れる値（`camRes` / `detW` / `markerIntervalMs` / `handDetW` / `bodyDetW`）は既にあるが、pixelRatio・フレームレート・推論の間引き・非表示時の停止は無い。運用面（画面の明るさを下げる・ケースを外す・充電しながら使わない・セッションを短く区切る。08 の対戦 1 分はこの意味でも合っている）で凌いでいる。負荷を下げる試行錯誤はライブラリ化の後で、ノブが 1 か所に集まってから全デモで一斉に試す方針
- **SDK ならどう解決するか（案）**: (1) 計測を SDK が持つ: 描画のフレーム時間・推論時間・検出時間・映像フレーム間隔を公開する（09 の HUD の `infer=..ms every ..ms` を全デモに広げたもの）。温度は読めないので、これらの悪化（フレーム時間の伸び・推論間隔の広がり）から熱制限を間接的に検知する (2) 品質のノブを 1 か所にまとめる: pixelRatio 上限・目標フレームレート・推論の間引き（N フレームに 1 回）と入力解像度・検出解像度と間隔・カメラ解像度を「品質段階」として SDK に持ち、アプリは段階を選ぶだけにする。どの段階を選ぶかの判断（ゲームごとのトレードオフ）はアプリ、手段と計測は SDK、という分担 (3) 画面非表示・ゴーグルから外した時（センサーが止まった時）にカメラと推論を止める (4) 実機で「何分で熱制限に入るか」「その時どの数字が悪化するか」を計測してから、効く順に手段を試す（fill 量 → 推論頻度 → カメラ解像度、の見立て）
- **関連**: `demos/08-splatoon/main.ts:379`（pixelRatio）、`demos/09-person-id/main.ts:772`（推論時間の HUD）、`src/shared/landmarker-tracker.ts` の `detect`（`inputMaxSide`）、`src/shared/marker-anchor.ts` の `update`（`getImageData` と `detMs`）、`src/shared/passthrough-camera.ts` の `openBackCameraStream`（`camRes`）。マーカー検出解像度の項（「常時高解像度は電池と発熱で不利」）の続き

## [2026-09-05] Phase 8 マルチマーカー / 08-splatoon: フェイクカメラの「画像を貼るだけ」では複数マーカーの位置合わせを PC で検証できず、ピンホール投影の 3D 版を作ることになった

- **何が苦しかったか**: 02〜08 のフェイクカメラは「マーカー ID 0 の SVG を画面の決まった位置に drawImage する」だけで、姿勢推定は「その位置・大きさに見える正面のマーカー」として成立していた。マルチマーカーでは、各マーカーから出した原点の姿勢が一致するかが検証したいことそのものなので、全マーカーが**同じ 1 つの視点から見た幾何**で描かれていないと意味がない（画像の重ね貼りでは床のマーカーの台形も、2 枚の相対位置も作れない）。さらに検出側の焦点距離の換算（長辺 / 2 / tan(FOV/2)、FOV はカメラのラベルから推定）と描く側の焦点距離を一致させないと、距離が定数倍ずれる
- **どう対処したか**: field 座標系に置いたマーカーを 1 セルずつ多角形で投影して塗る `src/shared/fake-markers.ts` を作り、従来の `fakeMarkerPx` / `fakeShift` / `fakeShiftY` は「原点マーカーがその大きさ・位置に映るカメラ位置」に換算して互換を保った（既存のヘッドレス確認はそのまま通る）。マーカーのビット列は検出と同じ辞書（js-aruco2 の `AR.Dictionary`）から取り、境界モジュール `marker-detector.ts` の `markerBits` / `markerSvg` として公開した（印刷ページ `markers.html` も同じ出所）。ヘッドレス確認では「2 枚同時 → spread < 5cm」「原点を隠しても ID 5 だけで self が動かない」「床のマーカーだけを見下ろす端末の self がフェイクカメラの位置に 1cm で一致」が取れた。実機の POSIT 精度（斜めの壁・遠いマーカー）は別問題で未検証
- **SDK ならどう解決するか（案）**: `@mobile-mr/spatial` のテスト支援として「シーン定義（マーカーの配置 + カメラ軌道）→ 合成映像」を持ち、実カメラの代わりに差し込める `FakeCameraSource` を標準で提供する。検出側と同じ内部パラメータ（焦点距離の換算）をシーン定義から自動で揃える
- **関連**: `src/shared/fake-markers.ts`、`demos/08-splatoon/main.ts` の `fakeWorld` / `FAKE_FOCAL_PX`、`scripts/headless-splatoon.mjs` のマルチマーカー節、PAIN_POINTS「カメラの焦点距離（内部パラメータ）がブラウザから取得できず」（Phase 3）

## [2026-09-05] Phase 8 マルチマーカー / 08-splatoon: マーカー間の相対位置を取る手段が「人間が巻き尺で測って入力する」しかなく、貼りズレを見つける診断表示が必須になった

- **何が苦しかったか**: WebXR 抜きのスマホブラウザには平面検出も VIO も無いので、追加マーカーの「原点からの位置」を自動で得る手段が無い。2 枚同時に見えた瞬間に相対変換を記録する自動キャリブレーションも考えたが、POSIT の誤差（斜め・遠いと数 cm〜十数 cm）がそのまま配置に固定されるうえ、誤差が連鎖する。結局「貼った面（向きは面から決める）+ 原点からの位置を実測して俯瞰画面に入力」の手動方式にした。すると今度は「見え方がずれたとき、貼りズレなのか・入力ミスなのか・推定誤差なのか」が切り分けられない
- **どう対処したか**: (1) 各マーカーの枠と ID を配置どおりの位置に描き、別のマーカー越しに見て実物と重なるかで貼りズレ / 入力ミスを見つける (2) 2 枚同時に見えたときの原点の候補のばらつき（spread [m]）と使用中の ID を HUD と pose（`markerIds`）に載せ、俯瞰画面の一覧で各自がどのマーカーで位置合わせしているかを見せる (3) 床のマーカーの高さは入力させず「マーカーの高さ（floorDrop）」から決める（入力項目を 1 つ減らし、寸法変更にも追従させる）。同時に見えた候補は画面上の辺長の 2 乗で重み付き平均（POSIT の並進誤差の分散が辺長の 2 乗に反比例するため）
...
## [2026-09-07] Phase 10-2 / 10-2-batting: Joy-Con の方向ボタンと IMU の「同時入力」は、インパクト時点のボタン状態だけでは取りこぼす

- **何が苦しかったか**: 球種を「方向ボタンを押しながら投げる」で選ぶが、Joy-Con の入力レポート 0x30 が持つボタン値は各レポート時点の状態だけで、振り全体に属する操作としては届かない。投手が振り始めに方向を押し、`SwingDetector` がインパクト（構えからの回転角が 0 を横切る瞬間）を検出する前に指を離すと、その瞬間のレポートは無入力になりストレートとして処理される。ボタンと IMU は同じレポートに入っていても、意味上の有効期間が違う
- **どう対処したか**: バッティング俯瞰画面の Joy-Con ごとに `selectedPitch` を持ち、投球の振り中に観測した最後の方向をボタンリリース後も保持する。インパクトではラッチした球種を送り、次の静止した構え（`address`）でストレートへ戻す。球種の決定とラッチはゲーム固有なので、Phase 10 の `SwingDetector` 自体には混ぜなかった
- **SDK ならどう解決するか（案）**: 将来の `InputSource` は生の `buttons` / `imu` だけでなく、時間窓を持つジェスチャーへ離散入力を関連付ける仕組み（例: `gesture.metadata` にジェスチャー中の最後の方向、押下エッジ、最大値を集約）を提供する。どの方向を何に使うかはゲーム側に残し、「インパクト瞬間の状態だけを見ると失う」という時系列処理を共通化する
- **関連**: `demos/10-2-batting/overview.ts` の `selectedPitch` / `onReport` / `onImpact`、`src/shared/batting-sim.ts` の `pitchTypeFromButtons`、`src/shared/swing-detector.ts`

## [2026-09-07] Phase 6-2 / 06-2-darts: 現実の手はパススルーの「絵」でしかなく、手より奥にあるはずのボードが手の前に描かれる（issue #14）

- **何が苦しかったか**: パススルー背景は `scene.background` の VideoTexture で、深度を持たない一枚の絵。背景に映っている自分の手は three にとって物体ではないので、ダーツボード・ダーツ・マーカー枠など仮想物は距離に関係なく必ず手の上に描かれ、「壁のボードが 0.4m の手より前にある」ように見える。手の骨格（SkeletonView）は `depthTest: false` で最前面に出すので骨格は見えるが、その下の実際の手はボードに埋まったまま。ブラウザからは深度センサー（LiDAR）も取れず、WebXR depth-sensing は iOS Safari 非対応。仮想物同士の前後は深度バッファが解決してくれるのに、現実の物体との前後関係は自分で「現実の物体の深度」を作って書き込む以外に方法が無い
- **どう対処したか**: 手の 3D 情報として唯一持っている MediaPipe の 21 点（hand-slots.ts がカメラ座標系に置いたもの）から、関節の球 + 骨の円柱 + 手のひらの多角形を実寸（大人の手の概算半径 + 余白 `?occluderPad=`）で組み、`colorWrite: false` で深度だけを書く（`src/shared/hand-occluder.ts`。`renderOrder = -1` で仮想物より先に描く）。手の奥にある仮想物の画素は深度テストで落ち、背景（= 現実の手）が残る。加えて、この層を Phase 5 の痛点「単眼パススルーの背景には視差が無い」の案 (a) どおり**視差ゼロ**で描く: 背景は両眼同じ画像なのに、カメラの子に置いた手のメッシュは eyeSep 分の視差を持ち（0.4m で片目 ±3cm ≈ 指 1 本）、そのままだと穴が左右の目で指 1 本分ずれて実際の手を覆い切れない。StereoEffect は目ごとの描画に cameraL/R を渡してくるので、`onBeforeRender` で目のカメラと中心カメラの位置差を取り、頂点シェーダ（`project_vertex` を `onBeforeCompile` で差し替え）でその差ぶん頂点をずらして、両眼で同じ画面位置に描く。ヘッドレス Chrome で穴の重心が左右の目で一致（視差ありだと 82px ずれる）することを確認。`?occluderDebug=1` で緑の加算合成にして実際の手との重なりを実機で見られるようにした（HUD が読めないゴーグル内での較正手段）。半透明（`transparent: true`）にすると three の描画順が不透明物の後ろに回って遮蔽が消えるので、不透明のまま加算合成にしている。**実機は未確認**（余白の既定 5mm・手の半径の概算が実物に合うかは iPhone で要確認）。既知の残り: メッシュは手の形を太らせた近似なので指の間や輪郭に数 mm のはみ出し・欠けが出る / 手首より先（腕）は隠さない / 相手の手・体は対象外
- **SDK ならどう解決するか（案）**: 「現実の物体の深度を書く層（オクルーダー）」を SDK の一級機能にする。手（21 点）・体（33 点）・マーカーの壁面（Surface）を同じ口で登録すると、深度だけの描画・仮想物より前の描画順・背景と同じ視差ゼロ・デバッグ表示を SDK がまとめて引き受ける。将来 WebXR depth-sensing や LiDAR が使える環境では同じ層を深度マップで差し替える。ステレオ描画側の「視差を付けないレイヤー」（Phase 5 の案）はこの層の実装で必要になった機能そのもの
- **関連**: `src/shared/hand-occluder.ts`、`demos/06-2-darts/main.ts` の `handOccluders` / `updateHandOccluders`、PAIN_POINTS「[2026-08-21] 単眼パススルーの背景には視差が無く…」、[issue #14](https://github.com/na2kera/mobile-mr/issues/14)

## [2026-09-11] 調査 / issue #53: 「計測」アプリ相当のこと（現実の長さを測る）がブラウザからはできず、定規はマーカーしか無い

- **何が苦しかったか**: 「iPhone の計測アプリのように長さが測れれば、コート寸法やマーカー位置をもっと正確にできる」（issue #53）を調べたところ、計測アプリを成り立たせている部品（ARKit の VIO による 6DoF とメートルスケール・平面検出・hit test・Pro 機種の LiDAR・較正済みの内部パラメータ）は**どれも iOS Safari に露出していない**。WebXR の AR 系（`immersive-ar` / hit-test / depth-sensing / plane-detection）は 2026-09 時点で iOS Safari に無く（Apple が出しているのは visionOS の `immersive-vr` だけ。2024-01 の Apple の回答は「AR セッションは未対応で議論も無い」）、Android Chrome にはある（ARCore 経由）。つまり「同じ URL を開いた iPhone と Android で、測れるかどうかが違う」。代替も揃わない: `getUserMedia` は焦点距離を返さない、DeviceMotion の積分は数秒で発散、WebAssembly の SLAM（8th Wall）はオープンソース化後も SLAM 本体がバイナリのみ、App Clip 経由の ARKit は「ブラウザだけ」の枠外、単眼深度推定（Depth Anything V2）はブラウザで動くが相対深度で Metric 版は未対応。PAIN_POINTS「ブラウザだけがハードウェアから隔離されていて」「カメラの焦点距離（内部パラメータ）がブラウザから取得できず」の 3 回目の再会で、今回は「測位」でも「姿勢推定」でもなく「計測」という機能そのものが作れない
- **どう対処したか**: 調査のみ（`docs/browser-measure-notes.md`）。結論は「計測は**焦点距離が取れない**と**6DoF が無い**の 2 つに帰着する」で、前者は「既知距離からマーカーを映す較正」で潰し、後者は「マーカーを貼った面の上に限って視線で点を置く」（07 の Surface と同じ数学）で回避する案を優先順に並べた。同じフレームに 2 枚映ったマーカーの左右方向の間隔は焦点距離の誤差に効かれない（POSIT の並進で f が約分される）ので、マルチマーカーの「相対位置は手動計測」を自動化する案 1 が最小。**未着手**（案の実装は別 issue）
- **SDK ならどう解決するか（案）**: 「計測」を SDK の一機能にする: (1) `mr.calibrate.camera()` — 既知寸法・既知距離のマーカーから焦点距離を較正して端末ごとに保存（機種 DB の種） (2) `mr.measure.onSurface(surface)` — 既知の面（マーカー由来 Surface、将来は WebXR の平面）の上に視線で点を置いて距離を返す (3) `mr.markers.calibratePair()` — 同時に見えた 2 枚から相対配置を推定して `config.markers` に流す。WebXR hit-test が使える環境（Android Chrome、将来の iOS）では (2) の面を「検出した平面」に差し替えられる口にする。6DoF（VIO）は段階 2 の自力実装か外部プラグインの範囲で、SDK の計測 API はそれが無くても面の上だけで成立する形にしておく
- **関連**: `docs/browser-measure-notes.md` / issue #53 / CONCEPT.md Phase 3「将来の拡張案：マルチマーカー」の自動キャリブレーション / `src/shared/marker-anchor.ts` の `extraMarkers` / `src/shared/surface.ts`（レイと面の交点）

## [2026-09-11] Phase 8 / 08-splatoon: 1 枚のマーカーから出した「傾き」は信用できず、床が 50cm 浮き壁が 50cm 手前に来る（issue #54）。重力（ジャイロ）で水平に直した

- **何が苦しかったか**: 実機で「マーカーがよく見えていても仮想の床が 50cm ほど浮く・マーカーの壁が 50cm ほど手前に出る」（issue #54）。距離の推定ではなく**回転（ピッチ）**の誤差だった。平面 4 点の POSIT（js-aruco2 posit1）は、ほぼ正面から見た四角形の傾きをほとんど決められない: 傾き θ は台形の潰れ cos θ から出るので、辺長の 1% の誤差で約 8° 動く。合成実験（検出側と同じ換算で 150mm を 1.5m から見る = 辺 35px、角に ±1px のノイズ）では傾きの誤差が中央値 9°・p90 で 35°、しかも POSIT の 2 解（best / alternative は鏡像の傾き）の誤差が僅差なので **best が鏡像側になる観測が 1〜2 割**あり、そのときは真値の 2 倍ずれる。傾きが 20° ずれると、マーカー中心を軸に field が回るので壁の下端（1.2m 下）は 0.4m 手前、足元（壁から 1.5m）の床は 0.5m 浮く — 症状そのもの。距離のほうは −1〜2% で問題なかった。追加マーカー（床など）では「マーカー → 原点」のオフセット（1m 超）を傾いた回転で戻すので原点の位置まで 0.3〜0.4m ずれる。PC のフェイクカメラは角がノイズ無しの完璧な四角形なので再現せず、実機でしか見えなかった
- **どう対処したか**: 端末には重力の向きが 1〜2° の精度で分かっている（DeviceOrientationControls のワールドは Y が鉛直上）。壁のマーカーは天地を合わせて貼る前提なので、各観測から出したアンカー（原点マーカー）の姿勢の Y 軸を重力の上に固定し、マーカーからは位置とヨー（上まわりの向き）だけ採る（`marker-layout.ts` の `levelRotation` / `tiltDegOf`、`marker-anchor.ts` の `worldUp`）。位置は「マーカー中心 − R_level・(原点系でのマーカー位置)」で、追加マーカーのオフセットも水平化した回転で戻す。POSIT の 2 解を「Y が上に近いほう」で選び直すことも試したが、横から見たマーカー（正面の壁の 2 枚目）では 2 解の差がヨー（重力では決まらない）で傾きは同じなので、ノイズの僅差で鏡像のヨーを拾って原点が 0.47m ずれた（ヘッドレスで検出）→ 水平化だけにした。鏡像の傾きは水平化で消えるので、正面付近では選び直しは不要。フェイクカメラでは合成カメラの姿勢から「上」を作って同じ経路を通し、ヘッドレス確認は原点マーカーを 25° 傾けて描いた映像で「水平化ありなら自分の位置が合う / なし（`?gravityAlign=0`）なら 0.3m 以上ずれる」を取った。PC + 実カメラ（OrbitControls）は重力とワールドが対応しないので off のまま。HUD に `tilt=`（水平化前の傾き）を出し、常に大きければマーカーが傾いて貼られていると分かるようにした。**実機は未確認**。残る限界: ヨーも同じ機序で ±8° 程度ぶれる（正面の壁は動かないが側面の壁が振れる）。ヨーは重力では決まらないので、複数マーカー・磁気センサー・壁の縁の検出などが要る
- **併せて見つかった罠**: 08 の印刷ページ `markers.html` の既定が 150mm、デモと俯瞰画面の `?markerMm=` の既定が 100mm で食い違っていた。印刷どおり貼って URL に何も付けないと距離が 2/3 になり、壁が 1.5m 先なら 50cm 手前に来る（症状が同じなので切り分けにくい）。`DEFAULT_MARKER_MM`（150）を `marker-layout.ts` に 1 つ置いて印刷・デモ・俯瞰画面で共有した。同じ物理量の既定値が 2 か所にあってはいけない
- **SDK ならどう解決するか（案）**: マーカーの姿勢推定は「画像だけの 6DoF」ではなく、**重力（IMU）を事前知識として持つ推定器**を標準にする（`MarkerField` に各マーカーの「面（壁 / 床）」を持たせているので、どの軸が鉛直かは分かっている。壁 = Y、床 = Z）。観測ごとに「位置（良い）・ヨー（普通）・傾き（悪い）」の不確かさを分けて返し、傾きは IMU、位置とヨーはマーカー、と信頼できる源を混ぜる融合を SDK 内で行う。POSIT の鏡像問題は「2 解のうち重力と矛盾しないほう」だけでは解けない（ヨーの鏡像は重力で見分けられない）ので、複数マーカー・時間方向の一貫性・視差の強い近距離の観測を組み合わせて解く推定器が要る。マーカーの実寸は「印刷ページが生成した ID と実寸を QR や URL に埋めて配る」形にして、利用者に 2 か所へ書かせない
- **関連**: `src/shared/marker-layout.ts` の `levelRotation` / `tiltDegOf` / `DEFAULT_MARKER_MM`、`src/shared/marker-anchor.ts` の `worldUp`、`demos/08-splatoon/main.ts` の `worldUp` / `?fakeTilt=`、`scripts/headless-splatoon.mjs` の「重力での水平化」節、PAIN_POINTS「[2026-08-16] カメラの焦点距離（内部パラメータ）がブラウザから取得できず…」、[issue #54](https://github.com/na2kera/mobile-mr/issues/54)

## [2026-09-11] Phase 8 / 08-splatoon: 追加マーカーを見ていると「補正されている感がない」— 傾きの誤りは原点マーカーでは無害だが、追加マーカーでは腕の長さで 1〜2m に増幅される（issue #55）

- **何が苦しかったか**: マルチマーカー（issue #30）で床や横の壁のマーカーを見ているとき「補正されている感がない。本当に補正されている？」（issue #55）。ロジック（anchor→world = marker→world × (marker→anchor)⁻¹）は正しく補正自体は毎回行われていた。原因は上の issue #54 と同じ POSIT の傾きの誤り（表裏 2 解の取り違え）だが、**効き方が違う**: 原点マーカーではマーカー自身が原点なので取り違えても位置は 3〜6cm しかずれず、単一マーカーの Phase 3〜8 では「少し傾く」程度で済んでいた。追加マーカーでは「マーカー → 原点」の腕の長さ（1〜2m）で傾きの誤りが増幅され、立って 100mm の床マーカーを見下ろす幾何（画面上 30px 前後）で原点が数フレームに 1 回 p90 1.9m 飛ぶ（Node で同じ POSIT にピンホール投影 + 角のノイズ 0.5px を入れて計測）。lerp で平均されて意味のない位置に落ち着くので「補正されていない」ように見える。合成カメラは角が正確でこの取り違えが起きないため、既存のヘッドレス確認（床マーカーを 0.5m の至近から）では出ず、実機でしか出ない
- **どう対処したか**: 対処そのものは issue #54 の重力での水平化（`marker-anchor.ts` の `worldUp`、`marker-layout.ts` の `levelRotation`。マーカーの中心は信用し、傾きを重力に合わせて原点までのオフセットを水平化した回転で戻す）で同じ幾何が p90 0.05m・最大 0.07m に収まる（IMU の重力誤差 3° を入れても 0.10m）。issue #55 側では (1) この幾何を `scripts/test-splatoon.mjs` の回帰テスト（実際の POSIT + 決定的ノイズ: 従来 p90 > 0.5m、水平化で < 0.15m）として残し、(2) HUD の `marker=` に `Δ=`（その観測がアンカーを動かした距離 = 補正量）を出して「本当に補正しているか」を実機で数字で見られるようにした。POSIT の 2 解を「重力に合う（傾きの小さい）方」で選び直す案も計測したが、水平化に対する上乗せは斜めの壁マーカーの最大値（2.9m → 0.1m）だけで p90 は変わらず、#54 の検証で「横から見たマーカーは 2 解の差がヨーなので鏡像のヨーを拾う（原点 0.47m ずれ）」と分かったため採用しない。**実機は未確認**。残る誤差: 正対した壁マーカー（左右・背面）のヨー誤差（p90 5〜13°）× 腕の長さ = 20〜40cm。マーカーを大きく（150mm）・近くで見ると減る
...
## [2026-09-16] Phase 8-2 / 08-2-splatoon-fixed: 「観測を採用するかどうか」を後から差し込む口が marker-anchor に無く、出力先を偽のアンカーにして公開値と文字列（`info` の `err=`）を読み取る回り道になった

- **何が苦しかったか**: 候補 5b「一度合わせて固定」は「観測は今までどおり取るが、アンカーに書くかどうかは別の方針で決める」だけの変更なのに、`createMarkerAnchor` は検出 → 融合 → lerp / スナップ → `anchor.position` への書き込みまでを 1 つの関数で抱えていて、「観測を受け取るだけ」の口が無い。過去のデモを触らない方針で `src/shared/marker-anchor.ts` は変更できないので、出力先に**観測用の Object3D**（`observed`）を渡し、`smooth: 1`・`resnapAfterMs: 0`・`snapDistanceM: 0` で「毎回スナップ = 生の観測」にしてから、`lastAcceptedMs` の変化で「新しい観測が来た」を検知して読み取る構造になった。しかも位置合わせの信頼判定に要る「再投影誤差」は公開値に無く、HUD 用の `info` 文字列（`"id=0 err=0.03,0.05 ..."`）を正規表現で読むしかなかった。マーカーの画面上の大きさ・中心位置（`sidePx` / `corners`）も同様に取れないので、観測したアンカーの姿勢とカメラの姿勢から「距離・画面中央からの角度・正対の角度」を逆算している（`fixed-anchor.ts` の `qualityOf`）
- **どう対処したか**: 上記の回り道（demo 内の `fixed-anchor.ts` / `fixed-anchor-math.ts`）。動くが、`info` の書式が変わると信頼判定が黙って壊れる（`err=` が読めないと Infinity = 信頼しない側に倒しているので、症状は「いつまでも確定しない」）。**未解決**: marker-anchor 側に「観測（ID・姿勢・誤差・画面上の大きさ）を返すだけの関数」を分けるのが本筋だが、過去デモを壊さずに足すには派生ファイルの複製になるので今回は見送った
- **SDK ならどう解決するか（案）**: 「検出 → 候補の姿勢（+ 質: 再投影誤差・画面上の大きさ・視角・距離）」と「候補をアンカーにどう反映するか（即時 / 平滑 / 一度合わせて固定 / 外部の位置と融合）」を別の層にする。前者は純粋な関数（画像 + 内部パラメータ → 観測の配列）、後者は方針の差し替え可能なストラテジ。HUD 用の文字列は最後に組み立て、数値はすべて構造化して返す。「観測の質」を SDK が標準で数値化しておけば、5b のような割り切り方式も、1a の重み付き最適化も、同じ入力から作れる
- **関連**: `demos/08-2-splatoon-fixed/fixed-anchor.ts` の `qualityOf` / `maxErrOf`、`src/shared/marker-anchor.ts` の `info` / `usedIds` / `tiltDeg`、`docs/space-stability-options.md` §4 5b

## [2026-09-16] Phase 8-2 / 08-2-splatoon-fixed: PC のフェイクカメラでは「斜めから見た観測の誤差」が再現できず、方式の効果（08 なら揺れる → 08-2 なら止まる）を PC で示せない。示せるのは「観測が動いてもアンカーが動かない」まで

- **何が苦しかったか**: 5b の価値は「斜めの観測は信用しない」ことだが、合成カメラ（`fake-markers.ts`）は角がノイズ無しの完璧な四角形なので、斜めから見ても POSIT の答えは正しく、08 でもコートは揺れない。ヘッドレスで比較実験の差を出そうとすると「観測にわざと誤差を入れる」道具が要るが、フェイクカメラにはそれが無い（`?fakeTilt=` はマーカーを傾けて描くだけで、角のノイズ・斜めの弱さは作れない）。結局ヘッドレスでは「確定後にカメラを 39° 斜めにする / マーカーを隠す / 10cm 動かす → 観測（`obsΔ=`）は動くがアンカーは 1µm も動かない」「`refine` で単調に寄る」という**機構**の確認に留め、方式の効果そのものは実機待ちになった。加えて 08 のフェイクカメラは姿勢を起動時に 1 回しか決めない（`fakeWorld()` を stream 作成時に呼ぶ）ので、「確定 → その後カメラを動かす」を再現するために姿勢を毎フレーム `window.__fakeCam` から作り直すよう demo 側で変えた
- **どう対処したか**: 上記のとおり機構の確認のみ。`obsΔ=`（観測とアンカーの差 = 08 ならこのぶん動いていた量）を HUD に出し、実機で 08 と同じ場面を見て数字で比べられるようにした。**未解決**: 合成カメラに「角の画素ノイズ」「斜めのときの検出の弱さ」を入れる仕組み（`scripts/experiment-posit-tilt.mjs` の Node 側にはノイズ入りの計測があるが、ブラウザのフェイクカメラには無い）
- **SDK ならどう解決するか（案）**: 合成カメラ（テスト用のフェイク入力）を SDK の一級機能にし、「実行中に姿勢を動かせる」「角に決定的なノイズを乗せられる」「斜め・遠い・小さいで検出が落ちる」を再現できるようにする。方式の比較（08 vs 08-2 vs アウトサイドイン）を同じ合成入力で回せれば、実機の前に候補を絞れる
- **関連**: `demos/08-2-splatoon-fixed/main.ts` の `fakeCamState` / `fakeCamPose`、`scripts/headless-08-2-fixed.mjs`、`src/shared/fake-markers.ts`、PAIN_POINTS「[2026-09-11] 追加マーカーを見ていると「補正されている感がない」…」（合成カメラでは再現しないと書いた同じ問題）

## [2026-09-16] 08-3 アウトサイドイン / 08-3-splatoon-outside-in: PC の Web カメラには IMU が無く、原点合わせ（1 枚の平面マーカーの姿勢）の傾きの誤差を水平化で救えない。その誤差が全員の位置に乗る

- **何が苦しかったか**: アウトサイドイン（docs/space-stability-options.md §4 の 2a）は「全員を同じ 1 台のカメラで測る」ので相手との相対位置は構造的に合うが、コート（field 座標系）と部屋の対応は「カメラが原点マーカーを一度見て決める」1 回の姿勢推定に全部かかる。その推定は 08 と同じ js-aruco2 の POSIT（平面 4 点）で、issue #54 で分かったとおり傾き（ピッチ / ロール）を 10〜30° 外し得る。08 のスマホ側はジャイロの重力で Y 軸を固定して救えたが、PC の Web カメラには IMU が無く、カメラを三脚で 35° 見下ろすように置くので「カメラは水平」の仮定も使えない。傾きが 10° ずれると、原点から 2m 離れた人の位置は 35cm ずれ、しかも全員が同じ向きにずれる（相対位置は保たれるが、壁・床との対応がずれる = インクが壁の別の場所に着弾する）。フェイクカメラは角がノイズ無しの完璧な四角形なので PC では再現せず（原点確定の誤差 1〜2cm・ヨー 1°）、実機でしか分からない
- **どう対処したか**: (1) 確定は直近 10 サンプルの重み付き平均（`fusePoseCandidates` を流用）にして画素ノイズは減らす（鏡像解の取り違えは平均では消えない）。(2) 原点候補を「正面の壁のマーカー」だけでなく 08 の「追加マーカーの配置」（床のマーカーなど、field 座標系での姿勢が分かっているもの）にも広げ、カメラを壁側に置いて部屋を向けても原点合わせができるようにした（床のマーカーは見下ろすカメラから正対に近く映るので条件が良い）。(3) スマホ側で壁のマーカーの枠を配置どおりに描き、実物と重ならなければ原点合わせがずれていると分かる診断にした。傾きの補正そのものは**未解決**（実機で誤差を測ってから: 床 + 壁の 2 枚を同時に見て合成する / 「床のマーカーの法線 = 鉛直」を拘束に使う / ChArUco 板を持ち歩いて確定する、のどれか）
- **SDK ならどう解決するか（案）**: 「外部カメラ（IMU 無し）の原点合わせ」を `spatial` の一機能にして、複数の既知マーカー（床 + 壁）の同時観測から 1 つの姿勢を最適化し、床のマーカーの法線 = 重力という事前知識をカメラ側の「上」として使う（08 の `worldUp` に相当するものをマーカーの面の種別から作る）。確定の品質（サンプルのばらつき・傾きの不確かさ）を数値で返し、「もっと近くで / 正対して映してください」と案内できるようにする。実機でしか出ない誤差を Node で再現する道具（POSIT + 投影 + 角のノイズ。issue #55 の回帰テストの形）はここでも要る
- **関連**: `demos/08-3-splatoon-outside-in/tracker.ts` の `lock` / `originToFieldOf`、`src/shared/outside-in-track.ts` の `camToFieldFromOrigin`、PAIN_POINTS「[2026-09-11] 1 枚のマーカーから出した「傾き」は信用できず…（issue #54）」、docs/space-stability-options.md §4 2a

## [2026-09-16] 08-3 アウトサイドイン / server: 役割（tracker）とメッセージ（track / tracked）を 1 つ足すために、08 のサーバー 370 行と game-client を丸ごとコピーした

- **何が苦しかったか**: `server/room-server.ts` の spec は「Room 設定・メッセージ・状態と tick」だけで、**役割（誰が何を送れるか）を知らない**。08 のサーバーは `?role=overview` を `Set` で持ち、`onMessage` の各分岐で `if (!isOverview) reject` を手で書いている。08-3 で tracker を足すには、その分岐を全部書き直す（`isPlayer` = overview でも tracker でもない、`playerIds` の除外、`canJoin` の役割別上限、`onLeave` の役割別処理）必要があり、08 を触らない方針なので 370 行をコピーして 60 行ほど足した。クライアント側も `game-client.ts` の `switch` にケースを足すために 180 行をコピーした。プロトコルの型は 08 の `ClientMessage` / `ServerMessage` を import して `Exclude` で welcome / join を差し替える形で拡張できたが、サーバーの `parseClientMessage`（検証）は export されておらず、これも 100 行コピーした。「メッセージを 1 種類足す」のコストが「サーバー 1 本ぶん」になっている
- **どう対処したか**: コピーで作った（方針どおり）。役割は `overviews` / `trackers` の 2 つの `Set` と `isPlayer()` で判定し、08 との差分がコメントで追えるようにした
- **SDK ならどう解決するか（案）**: Room サーバーの spec に**役割とメッセージの許可表**（`roles: { player: ["pose", "shot"], overview: ["start", …], tracker: ["track"] }`）と役割別の上限を持たせ、`if (!isOverview) reject` の分岐を spec から消す。メッセージの検証は「型ごとの検証関数の表」にして、拡張側は表に 1 行足すだけにする。クライアントの `connectGame` も「メッセージの型 → ハンドラ」の表で拡張できる形にする（`switch` のコピーを無くす）。PAIN_POINTS「Room サーバーのボイラープレートが 3 本目になった」の続きで、共通化した後にも「役割」という軸が残っていた
- **関連**: `server/splatoon-outside-in.ts`（`roleOf` / `isPlayer` / `canJoin`）、`src/shared/splatoon-outside-in-protocol.ts`、`demos/08-3-splatoon-outside-in/game-client.ts`、`server/room-server.ts` の `RoomServerSpec`

## [2026-09-16] 08-3 アウトサイドイン / tracker: 検出器がマーカーの実寸を 1 つしか持てず、原点用（150mm）とゴーグル用（60mm）を同じ映像で扱うのに POSIT の並進を比で直した。頭追従にも「位置は外から・回転はジャイロ」という混ぜ方の口が無い

- **何が苦しかったか**: (1) `createMarkerDetector(markerSizeM)` は POSIT を「実寸 × 焦点距離」固定で作るので、1 フレームに大きさの違うマーカーが混ざると（壁の原点 150mm・床 150mm・ゴーグル 60mm）検出器を 2 つ作って重い検出を 2 回走らせるか、並進を比で直すかになる。POSIT の並進が実寸に比例する（回転は不変）ことを Node のテストで確かめてから比で直したが、検出器の API がこの前提を表に出していない。(2) スマホ側は 08 の `createMarkerAnchor`（映像 → アンカー）を外して「サーバーから来る位置 + ジャイロの回転」でアンカーを組み直したが、`DeviceOrientationControls` はカメラの回転を直接書き、位置は `camera.position` に固定なので、「位置だけ別の源から」を素直に書く場所が無く、アンカー（field → ワールド）の側を毎フレーム逆算する形（カメラ位置 − R_y(φ)・自分の位置）になった。ヨーも「ジャイロのヨーとトラッカーのヨーの差 φ」を自分で持って緩やかに寄せる必要があり、位置の補間（遅延）・見失い時の保持・ヨーの補正の品質条件を全部デモ側で書いた
- **どう対処したか**: (1) 検出器は原点の実寸で 1 つ作り、ゴーグルの観測だけ並進を `trackMarkerMm / markerMm` 倍する（`tracker.ts` の `apply`。根拠は `scripts/test-08-3-outside-in.mjs`）。(2) `src/shared/outside-in-track.ts` に `anchorYaw` / `approachAngle` / `anchorPose` を切り出し、main.ts の `onTracked` / `updateAnchor` で混ぜる。位置は前回値からの lerp、見失いは最後の位置を保持して HUD に `tracked (0.8s ago)`
- **SDK ならどう解決するか（案）**: (1) 検出器は「単位マーカーの姿勢」を返し、実寸は ID ごとに後から掛ける（`MarkerField` が ID → 実寸を持つ）。(2) `HeadTracker` を「回転の源（ジャイロ）」と「位置の源（マーカー / 外部トラッカー / UWB）」の合成として設計し、位置の補間・保持・ヨーの融合（外部のヨーで IMU のドリフトを緩やかに補正する）を SDK 側に持つ。CONCEPT.md の「外付けハードによる 6DoF（UWB）」もこの口に入る
- **関連**: `demos/08-3-splatoon-outside-in/tracker.ts` の `apply`、`demos/08-3-splatoon-outside-in/main.ts` の `onTracked` / `updateAnchor`、`src/shared/outside-in-track.ts`、`src/shared/marker-detector.ts` の `createMarkerDetector`

## [2026-09-17] 08-5 / splatoon-apriltag: マーカーの検出器を差し替えるだけなのに、アンカー・印刷ページ・フェイクカメラの 3 か所が js-aruco2 に縛られていた

- **何が苦しかったか**: 候補 1c（`docs/space-stability-options.md`）は「検出器を AprilTag に替える」だけの実験のはずが、08 の構造では検出器が 3 か所に染み出していた。(1) `src/shared/marker-anchor.ts` の `createMarkerAnchor` は中で `createMarkerDetector`（js-aruco2）を作り、差し替え口が無い。観測 → 水平化 → 重み付き平均 → スナップ / lerp の 120 行は検出器と無関係なのに、検出器を替えるにはこの全部を写す（`demos/08-5-splatoon-apriltag/marker-anchor-apriltag.ts`）しかなかった。(2) 印刷ページ `markers.ts` と (3) フェイクカメラの絵柄は `marker-detector.ts` の `markerBits` / `markerSvg`（js-aruco2 の辞書）から取っていて、ファミリが違う AprilTag では符号表（tag36h11 の 587 個の 36bit 符号 + bit の座標）を C ソースから写して自前で持つ（`tag36h11.ts`）必要があった。公式の PNG と突き合わせるまで bit の順序（MSB から / 1 = 白 / bit_x, bit_y の並び）が合っている確信が持てなかった
- **どう対処したか**: アンカーは検出器を引数で受ける `createDetectorAnchor` を demo 内に写し、AprilTag と js-aruco2（比較用 `?detector=aruco2`）を同じアンカーに通す（違いは検出器だけ、が比較の前提）。既存ファイルは変更していない。絵柄は `tag36h11.ts` に符号表 0〜249（`MAX_MARKER_ID` と同じ範囲）を持ち、ID 0 / 1 / 5 が公式画像とセル単位で一致することを `scripts/test-08-5-apriltag.mjs` に残した。幸い tag36h11 の幾何（黒 8 セル + 余白 1 セル = 10 セル格子、タグの大きさ = 黒い正方形の一辺）が ARUCO_MIP_36h12 と同じで、`fake-markers.ts` の投影と `?markerMm=` の意味はそのまま使えた
- **SDK ならどう解決するか（案）**: `MarkerDetector`（画像 → ID + 4 隅 + 姿勢）、`MarkerFamily`（ID → 絵柄。印刷・合成・辞書の大きさ）、`AnchorEstimator`（観測 + IMU + 配置 → アンカー）を別の interface にし、アンカーは検出器を注入で受ける。ファミリは検出器が公開する（`detector.family.bits(id)` / `svg(id)`）ことで、印刷ページとフェイクカメラが検出器と食い違えない。フェイクカメラは「family + 配置 + カメラ姿勢 → 映像」の共通部品にする
- **関連**: `demos/08-5-splatoon-apriltag/marker-anchor-apriltag.ts`、`tag36h11.ts`、`fake-apriltags.ts`、`src/shared/marker-anchor.ts:102`、`src/shared/marker-detector.ts:58`

## [2026-09-17] 08-5 / splatoon-apriltag: AprilTag の姿勢の座標系は js-aruco2 と Y の向きが違い、変換の正誤は往復テストでしか分からない

- **何が苦しかったか**: AprilTag（apriltag_pose.h）はカメラ X 右・Y 下・Z 前、タグ X 右・Y 下・Z 奥。js-aruco2 の POSIT は「画像中心原点・Y 上」の角を渡すので、08 の `marker-detector.ts` は F = diag(1, 1, -1) で挟むだけで良かったが、AprilTag では F = diag(1, -1, -1)（Y と Z の両方を反転）で挟み、並進も (t0, -t1, -t2)。さらに WASM の JSON の `R` は「列優先」（`apriltag_js.c` のコメントにしか書いていない）、4 隅の順序は左下から反時計回り（`apriltag.c` の "FLIP Y" のコメント）、`e` は物体空間の誤差 [m²] で 08 の正規化再投影誤差（画素）とは流儀が違う。どれも実機で「壁が裏返る」「床が天井に行く」まで気づかない類で、ブラウザで動かして目で見ても符号 1 つの取り違え（鏡像）は判別しにくい
- **どう対処したか**: 純粋な変換を `apriltag-pose.ts` に切り出し、`scripts/test-08-5-apriltag.mjs` で往復テスト（合成の姿勢 → AprilTag の流儀で 4 隅を投影 → 変換した Matrix4 で投影、が角の順序込みで一致。手で導いた「上端を θ 傾けたタグ」の R から Y 軸が (0, cosθ, sinθ) になる。js-aruco2 の F を使う間違いでは再投影誤差が 0.5 を超える）。実行時も `e` ではなく自前の再投影誤差（4 隅を投影し直して辺長で割る）を `MarkerObservation.error` に入れるので、変換が崩れれば HUD の `err=` が 1 前後になって気づける（ヘッドレスで 0.00〜0.03）。同梱の Comlink + Web Worker ラッパー（`apriltag.js`。unpkg から Comlink を読む）は LAN のオフラインで動かないので使わず、cwrap の呼び出しを `apriltag-detector.ts` に書いた（Emscripten の MODULARIZE はグローバルを定義するクラシックスクリプトで ES module から import できず、`<script>` を差し込んで待つ。検出のたび stdout に `###Size:` を出すので `print` を潰す）。`raw.githubusercontent.com` は .js を gzip で返し `content-length` が圧縮後のサイズなので、`fetch-models.mjs` から写したサイズ照合が誤検知した（後に Codex レビューの指摘でサイズ照合をやめ、固定した SHA-256 の照合に置き換えた）
- **SDK ならどう解決するか（案）**: 検出器の出力は「SDK の規約（カメラ Y 上・-Z 前、マーカー Y 上・+Z 視点側、4 隅は左上から時計回り、誤差は正規化再投影誤差）」に境界モジュールで揃え、各検出器（js-aruco2 / AprilTag / OpenCV）ごとに往復テストを SDK のテスト資産として持つ。誤差の定義も SDK が決め、検出器固有の値（POSIT の画素和、AprilTag の物体空間誤差）は参考値として別フィールドに置く
- **関連**: `demos/08-5-splatoon-apriltag/apriltag-pose.ts`、`apriltag-detector.ts`、`scripts/test-08-5-apriltag.mjs`、`scripts/fetch-apriltag.mjs`、PAIN_POINTS「[2026-08-16] カメラの焦点距離（内部パラメータ）がブラウザから取得できず…」
...
- **どう対処したか**: プロトコルとサーバーは変えない方針（08 は触らない・比較のため同じプロトコル）なので、画面マーカーのページは一辺の入力が落ち着いたら新しい値で入り直し、拒否されたら理由と「全員の URL に ?markerMm= を付けて入り直す」案内を出すだけにした（`markers-screen.ts` の `connect()`、ヘッドレス確認で拒否の経路も取った）。収まらない一辺を丸めたときも同じ食い違いが起きる（既定の 150mm を 27 インチの multi で開くと 81mm で描くのに room は 150mm で、スマホは距離を 1.85 倍に見積もる。コードレビューで指摘）ので、丸めている間は接続も反映もせず、パネルに赤で「一辺 150mm は画面に収まりません。81mm 以下にするか、1 枚モードにしてください」と出す（room の設定を黙って変えないため自動で入り直さない。既定のモードは収まりやすい single）。**未解決**: 遠い / 近いの切り替えを試合中に行うことはできない
...
- **何が苦しかったか**: 印刷なら CSS の `mm` 単位がプリンタでほぼ実寸になる（08 の `markers.html` はこれ）が、画面では `1mm = 3.78 CSS px` 固定で物理サイズとは無関係。`screen.width` / `devicePixelRatio` はあっても物理的な対角は Web API に無い（Screen Details API にも無い）。macOS の「ディスプレイの解像度」（スケーリング）や Chrome のズームで CSS px の数も変わる。結果として「対角のインチ（型番の数字）」か「1mm あたりの px」を入力欄で入れてもらい、`hypot(screen.width, screen.height) / 対角 mm` で換算する以外に無く、入力ミス（インチと mm の取り違え・ズーム 125%）を自動で検出する手段がほぼ無い。当初は「追加マーカーが原点から 5m 超なら弾く」を入れたが、配置の mm は「一辺 × 比」で決まり px/mm に依らないので一度も発火しない検査だった（コードレビューで指摘）。さらに「1mm あたり px」が CSS px か物理画素か（Retina の拡大率 2 で 2 倍違う）も利用者には紛らわしい。ヘッドレス Chrome ではさらに `--window-size` とビューポートが一致しない（1280x720 指定で innerHeight は 577）ので、収まる最大の一辺が変わって配置がずれ、`Emulation.setDeviceMetricsOverride` で固定する必要があった
- **どう対処したか**: 対角インチ / CSS px 直接指定の 2 つの入力欄 + `localStorage`（保存時の `screen.width` / `screen.height` / `devicePixelRatio` を併記し、変わっていたら使わず入れ直してもらう）。換算した画面の幅が 150〜5000mm の外なら警告して反映しない（大きな取り違えだけ）。細かいずれは画面の隅に常時出す 100mm の較正線に定規を当てて確かめてもらう運用にした。純粋な変換（`screen-layout.ts`）を three 非依存で切り出して Node で検証。**未解決**: 実寸の自動取得
- **SDK ならどう解決するか（案）**: 「画面 / 投影面にマーカーを出す」を SDK の供給源の 1 つ（`MarkerSource.screen`）として持ち、実寸の較正を UI 込みで提供する: (1) 端末側で「既知の実寸の紙マーカーと画面のマーカーを同時に見る」と画面の px/mm が逆算できる（紙の姿勢から画面のマーカーの実寸を推定）、(2) 入力した対角と `screen` の px から換算して保存、(3) 換算の妥当性（画面の実寸が現実的な範囲か）を検証し、較正用の既知の長さを画面に出す。マーカーの実寸を「配置の一部」として配る前提（上の痛点）と組み合わせると、較正結果をそのまま全員に配れる
- **関連**: `demos/08-6-splatoon-screen-markers/screen-layout.ts` の `pxPerMmFromDiagonal` / `validateScreenSize` / `placeRuler`、`markers-screen.ts` の `currentPxPerMm`、`scripts/headless-08-6-screen-markers.mjs` の `Emulation.setDeviceMetricsOverride`、PAIN_POINTS「[2026-08-16] カメラの焦点距離（内部パラメータ）がブラウザから取得できず…」（同じ「物理量が Web API に無い」問題）

## [2026-09-17] 08-4 / 08-4-splatoon-opencv: 「推定の数学を替える」だけでは 1 枚の斜めマーカーは良くならず、効いたのは「複数枚を 1 つの board にする」ことだった

- **何が苦しかったか**: docs/space-stability-options.md 1b の検証として OpenCV.js（WASM 11MB）の `ArucoDetector` + `solvePnP` に位置の取り方を替えた（`src/shared/marker-anchor-opencv.ts`）。同じ合成画像を js-aruco2 + POSIT（08）と OpenCV + IPPE/LM（08-4）に流して比べると（`scripts/test-08-4-opencv.mjs`）、**原点 1 枚だけなら 45〜60° で p90 3.6〜5cm と同等**（正面〜30° では 2〜3 倍良い程度）で、「斜めの平面 4 点は条件が悪い」問題は推定器を替えても残る。一方、同じ壁の 2 枚を 8 点の board として 1 回の solvePnP で解くと p90 0.6〜0.9cm（08 の「1 枚ずつ推定して辺長² で平均」は 2.1〜2.3cm、正面で 2 枚目を斜めに見る条件では POSIT の鏡像で 25cm）。1 枚ずつの推定の平均は鏡像の解を平均してしまうが、board は鏡像を持たない。さらに 1 枚だけになった瞬間（原点を隠して 100mm の 2 枚目だけ）は IPPE も 2 解の誤差が僅差で、ヘッドレスで 0.46m 飛んだ。前フレーム（board の解）を初期値にした ITERATIVE を「再投影誤差が同程度なら」優先することで留まる（最初の取得が鏡像なら居座るので 2s ロストで初期値を捨てる）
- **どう対処したか**: (1) 全マーカーの 4 隅を field 座標系の対応点にまとめ、平面なら IPPE → LM、非平面（壁 + 床）なら ITERATIVE（前フレームを初期値）、平面でも前フレームがあれば初期値ありの解を連続性で優先（`opencv-pose.ts` の `solveBoardPnP`）。(2) 重力での水平化は残し、位置は「見えているマーカーの中心の平均 − R_level・(その平均のアンカー座標)」に一般化。(3) 診断に board の再投影誤差 `reproj=`（配置の入力ミス 30cm で 5px）を HUD に出す。(4) 比較の数値は README の表に残した。**実機は未確認**
- **SDK ならどう解決するか（案）**: `@mobile-mr/spatial` の姿勢推定は「1 枚ずつ → 平均」ではなく「見えている全観測を 1 つの最小二乗で解く」を既定にし、1 枚だけのときの 2 解は時間方向の連続性 + 重力 + （あれば）他の観測で選ぶ推定器にする。OpenCV.js は 11MB の WASM で iOS の読み込み・発熱が未知なので、SDK ではこの結果（board 化が効く）を根拠に平面 PnP（IPPE 相当）+ 全点最適化を自力実装する（1a）
- **関連**: `src/shared/pnp-board.ts`、`src/shared/opencv-pose.ts`、`demos/08-4-splatoon-opencv/README.md` の比較表、PAIN_POINTS「[2026-09-11] 1 枚のマーカーから出した「傾き」は信用できず…」「[2026-09-11] 追加マーカーを見ていると「補正されている感がない」…」

## [2026-09-17] 08-4 / 08-4-splatoon-opencv: OpenCV.js を「依存に足さず・script タグも使わず」読むと、UMD / Emscripten の癖（Module.then の無限ループ・ESM 扱いの __dirname）を全部踏む

- **何が苦しかったか**: 方針（package.json に足さない・`<script>` ではなく動的 import）どおり `public/vendor/opencv/opencv.js`（公式ビルドそのもの。Cloudflare の challenge で docs.opencv.org からは落とせず jsDelivr の npm ミラー経由）を fetch → Blob の ES モジュールで import した。すると (1) この Emscripten ビルドの Module には `then(func)` が生えていて、async 関数から返す / Promise で解決した瞬間に「thenable を解決 → then が Module 自身を渡す → また thenable」の無限ループになり、Node が CPU 100% で沈黙する（`--prof` で `PromiseResolveThenableJob` が 100% になって発見。ブラウザでも同じ）。(2) Node のテストで `require` すると、リポジトリが `"type": "module"` なので `.js` が ESM 扱いになり `__dirname is not defined` で落ちる（CommonJS の関数でくるんで評価する必要がある）。(3) Vite の dev サーバーは無いパスにも index.html を 200 で返すので、404 の検出には content-type を見る必要がある。(4) 合成画像で js-aruco2 と OpenCV を並べると、縁が鋭い画像（fillPoly）では js-aruco2 は候補を見つけても ID を読めず、fillPoly の縁は両端の画素を塗って黒い正方形が 1px 大きくなる（距離 3% 近く）など、「実写らしい合成」を作ること自体に 3 回の作り直しが要った（4 倍描画 + INTER_AREA + ぼかし σ 1.2 で両者とも 24/24）
- **どう対処したか**: `opencv-loader.ts` で `delete cv.then` してから返す。Node のテストは `new Function("module","exports","require","__dirname","__filename", src)` で評価。content-type が html なら失敗にして js-aruco2 へフォールバック。合成画像の描き方はテストのコメントに残した
- **SDK ならどう解決するか（案）**: 外部 WASM を「取得スクリプト + 実行時 fetch + フォールバック」で扱う型（`fetch:<name>` / `public/vendor/<name>/` / `load<Name>()` が進捗と失敗を返す）を SDK の作法として固定し、Emscripten の thenable / Node の ESM 判定はローダーが吸収する。比較用の合成カメラは「実写らしさ（AA・ぼけ・量子化）」を持つものを SDK のテスト資産にする
- **関連**: `src/shared/opencv-loader.ts`、`scripts/fetch-opencv.mjs`、`scripts/test-08-4-opencv.mjs` の `loadOpenCvNode` / `renderImage`、PAIN_POINTS「[2026-09-05] 合成カメラの…」（該当あれば）

## [2026-09-17] 08-7 / 08-7-splatoon-alva: 単眼 SLAM（AlvaAR）の姿勢は「座標系・スケール・状態」が API から読めず、ソースを読まないと field に写せない

- **何が苦しかったか**: AlvaAR の `findCameraPose(imageData)` は 16 要素の Float32Array か null を返すだけで、(1) それが Twc（カメラ → ワールド）か Tcw か、(2) 行列が行優先か列優先か、(3) 座標系が OpenCV 流儀（Y 下・Z 前）か、(4) null が「初期化中」「追跡失敗でリセットした（= 座標系が変わった）」のどちらか、が README に無い。C++ の `Utils::toPoseArray` を読んで「Twc・回転は行優先・位置は [12..14]・OpenCV 流儀」、`System::processCameraPose` を読んで「戻り値 1=追跡 / 2=リセット / 3=初期化中（JS のラッパーが捨てている）」と分かった。リセットを検知するには JS ラッパーの内部（`alva.system` / `memImg` / `memCam`）を直接叩くしかない。戻り値の配列は WASM ヒープへのビューで、次の呼び出しで上書きされる（複製しないと壊れる）。`AlvaAR.Initialize(w, h, fov)` の `fov` も「短辺側に縦横比を掛けた」独自の換算で、マーカー検出と同じ焦点距離にするには内部の `getCameraIntrinsics` を二分探索する必要があった。さらに Vite の dev サーバーは `public/` のファイルを動的 import すると `?import` を付けて 500 にする（`new Function("url", "return import(url)")` で回避）
- **どう対処したか**: 変換を three 非依存の `slam-fusion.ts` に切り出し、**生成側の関数を使わない手計算の固定期待値**（OpenCV の Y まわり +90° は three の Y まわり -90° で右を向く、等）でテストした。同梱の `alva_ar_three.js` の四元数 `(-x, y, z, w)` と位置 `(x, -y, -z)` とも一致を確認。状態はピン留めしたコミットの内部を使い、無ければ公開 API に落とす。GPLv3 なので npm には入れず、コミット固定 + SHA-256 照合のスクリプトで取得し、デモ限定にした。**実機は未確認**
- **SDK ならどう解決するか（案）**: 外部の追跡器（SLAM・WebXR・ARKit 経由・アウトサイドイン）を差し替えられる `PoseSource` の interface を持ち、「姿勢の意味（カメラ → ワールド、three 流儀、単位 m かスケール不定か）」と「状態（初期化中・追跡中・リセット = 座標系が変わった・停止）」を型で必ず返させる。座標系の変換は SDK 側で 1 か所にまとめ、手計算の固定期待値テストを付ける。GPL の実装は SDK に入れられないので、SDK はアダプタの口だけを持ち、アダプタは利用者側に置く
- **関連**: `demos/08-7-splatoon-alva/slam-fusion.ts` の `alvaPoseToThree`、`demos/08-7-splatoon-alva/slam-source.ts`、`scripts/test-08-7-alva.mjs` の「手計算」、`scripts/fetch-alva.mjs`、https://github.com/alanross/AlvaAR（`src/slam/src/utils.cpp` / `system.cpp`、コミット 7796af5）

## [2026-09-17] 08-7 / 08-7-splatoon-alva: 「マーカーで SLAM のスケールと向きを決める」は、マーカーの推定の遅れと鏡像解がそのまま変換に入る

- **何が苦しかったか**: 単眼 SLAM にはスケールが無いので、マーカーが見えている間の「マーカーで出したカメラの位置」と「SLAM の位置」の組から相似変換を推定した。ヘッドレス（フェイクの SLAM）で 2 つの罠に当たった。(1) アンカーは lerp（`smooth=0.5`）と検出間隔で 0.3〜0.5 秒遅れて追いつくので、動いた直後の組は「SLAM は新しい位置・マーカーは古い位置」になりスケールが縮む。(2) 合成カメラをマーカーの視軸から 15〜18° 横にずらすと単一マーカーの POSIT が鏡像のヨーを拾い、マーカー側の位置が 30° 回った場所に出て、推定したスケールが正解の 1/4（s=0.66、正解 2.70）になった。ロスト中に歩くと自分の位置が 1/4 しか動かない。マーカーを「原点だけでなく SLAM の較正にも」使うと、08 の原因 A（斜めの弱さ）が歩いた距離に掛かって増幅される。また PC の合成カメラでは 100mm のマーカーが 0.9m 以上で検出されず、較正の基線（前後の動き）を大きく取れない
- **どう対処したか**: 組にするのは「マーカーの採用直後」かつ「直近の補正量 Δ が小さい（アンカーが追いついている）」フレームだけにした（`?slamPairMaxCorr=0.08`）。回転は位置の軌道からではなく各組の向きの差の平均で出す（ほぼ直線の軌道では位置から回転が決まらない）。確認では視軸から 5° 以内・0.5〜0.8m の位置で較正した。基線の閾値は当初「重心からの距離の RMS」で、15cm 動いても 0.045 にしかならず閾値を下げていたので、軌道の直径で判定するように直した（既定 0.15 のまま較正される）。レビュー後、外れ値の除去（残差が中央値の 3 倍を超える組）と採用条件（残差 ≤ 0.35 × 基線・組 ≥ 10・スケールの変化 ≤ 20%）を足し、遅れ・反射相当の組では採用しないようにした（`scripts/test-08-7-alva.mjs`）。鏡像解そのものは未解決（08 と同じ）。HUD に `off=`（マーカーと SLAM を写した位置の差）と `rotErr=`（SLAM の向きとジャイロの差）を出して実機で壊れた組に気付けるようにした。**実機は未確認**
- **SDK ならどう解決するか（案）**: スケール・向きの較正を「平滑化済みのアンカー」ではなく**生の観測（タイムスタンプ付き）**と、同じ映像フレームの SLAM の姿勢で対にする（マーカー検出の API が観測と時刻を返す）。観測ごとの信頼度（視角・画面上の大きさ・再投影誤差・2 解の僅差）で重み付けし、外れ値を RANSAC で落とす。較正中は「マーカーの正面で前後に動いてください」と誘導し、基線と残差が十分になったら確定する UI を SDK の部品にする
- **関連**: `demos/08-7-splatoon-alva/main.ts` の `updateSlam`、`demos/08-7-splatoon-alva/slam-fusion.ts` の `estimateSimilarity`、`scripts/headless-08-7-alva.mjs` の「マーカーを見ながら動いて変換を推定させる」、PAIN_POINTS「[2026-09-11] 追加マーカーを見ていると「補正されている感がない」」、docs/space-stability-options.md §4 の 3a

## [2026-09-17] 08-8 Pose カメラ / tracker: 人物の検出からは「頭の向き」も「絶対距離」も直接取れず、体の仮定（肩幅・首の長さ・顔のオフセット）を積み重ねることになった

- **何が苦しかったか**: 2b（docs/space-stability-options.md §4。PC の Web カメラ + MediaPipe PoseLandmarker で人を追う）では、マーカー版（08-3）なら 1 枚のマーカーから出た「位置・向き・実寸」をすべて体の仮定で置き換える必要があった。(1) 距離: PoseLandmarker の `worldLandmarks` の尺度はモデルの推定で、09 と同じ最小二乗は「その尺度が正しい」前提なので、両肩の 3D 距離を `?shoulderM=`（0.4m）に合わせて拡大縮小した。肩幅が 10% 違えば距離も 10% ずれ、体格の違う人が混ざっても値は全員共通の 1 つ。(2) 向き: ゴーグルは顔の前にあり目が隠れるので、向きは両肩（+ 見えれば両耳）の「左 − 右」から出す = 体の向きで、首だけ回した頭の向きとは違う。スマホのジャイロのドリフト補正（08-3 の φ の緩やかな補正）が体の向きに引っ張られる。背を向けると MediaPipe の左右が入れ替わり 180° 反転し得る。(3) 位置: 頭の中心は両耳の中点を基本に、鼻（前 8cm・下 1cm）と目（前 6cm・上 2cm）のオフセットを引いた推定を可視性で重み付けし、耳が見えなければ肩の中点 + 首 0.25m。スマホのカメラはさらに正面へ 10cm。どれも「平均的な大人」の定数で、実機でゴーグルをかけた顔の鼻・耳の `visibility` がどう出るかは分からない
- **どう対処したか**: 仮定はすべて URL パラメータ（`shoulderM` / `neckM` / `eyeFwdM` / `poseMinVis`）に出し、`src/shared/pose-cam-track.ts` の純粋関数にまとめて Node で「手計算の固定期待値」（肩の向き → ヨー、耳と鼻の重み付き平均、向き合った人の左肩が画像の右）と「肩幅の仮定を 10% 変えると距離が 10% 変わる」を確かめた。ヨーの補正は 08-3 の品質の閾値と首振り中の停止をそのまま使い、品質 = 肩の見かけの幅 × 正対の度合いにした。体の向き ≠ 頭の向き・背面での反転・体格差は**未解決**（実機で誤差を測ってから）
- **SDK ならどう解決するか（案）**: 「外部カメラで人を追う」位置の源は、(a) 尺度の仮定をプレイヤーごとに較正する手順（入室時に原点マーカーの横に立つ・スマホ側の 08 のマーカー推定と一度突き合わせる）、(b) 向きは外部カメラからは使わず（または信頼度を低くして）、IMU のヨーのドリフト補正は別の源（マーカーを見たとき・相互補正）に任せる、の 2 つを API で分けて持つ。`HeadTracker` の「位置の源」に信頼度（位置の誤差の目安・ヨーを使ってよいか）を持たせ、マーカー / 人物検出 / UWB を同じ口で差し替えられるようにする
- **関連**: `src/shared/pose-cam-track.ts` の `shoulderScale` / `bodyYaw` / `headCenter` / `observePerson`、`scripts/test-08-8-pose-cam.mjs` の「固定値」、`demos/08-8-splatoon-pose-cam/README.md` の「2a との違い」、PAIN_POINTS「[2026-09-16] 08-3 アウトサイドイン / tracker: … 頭追従にも「位置は外から・回転はジャイロ」という混ぜ方の口が無い」

## [2026-09-17] 08-8 Pose カメラ / 割当: マーカーの ID が無くなると「誰がどのプレイヤーか」の状態をトラッカーの画面が持つことになり、見失い・再接続・原点の確定し直しのたびに儀式（手を挙げる）がやり直しになる

- **何が苦しかったか**: 08-3 はマーカーの ID = プレイヤーの番号だったので、見失っても再び見えれば自動で復帰した。08-8 は人物に ID が無いので、入室順に「手を挙げた人」に割り当て、以後は前フレームの位置との近さで追う（`PoseAssigner`）しかない。すると (1) 2 秒見失ったら別人に付かないよう未割当に戻す → 他の人に隠れただけでも挙げ直し、(2) 再接続（瞬断・bfcache）でプレイヤーの id が変わる → 挙げ直し、(3) 原点を解除して 2 秒以上経つと人物が消える → 挙げ直し、(4) トラッカーのページを再読み込みすると割当（ページのメモリ）が全部消える、(5) 2 人が近くですれ違うと入れ替わり得る、と「割当が外れる」経路が多い。同時に挙げた 2 人目が次のプレイヤーに流れ込まないように「直前の割当より前から挙げていた手は数えない」規則も要った。PC ではフェイクの体の `raised` を切り替えるだけなので、実際の挙手の誤検出（手を振る・頭をかく）や、すれ違いの入れ替わりの頻度は分からない
- **どう対処したか**: 割当はトラッカー（俯瞰画面）だけの状態にして、サーバーとプロトコルは 08-3 のまま（track の id にプレイヤーの番号を載せる）にした。俯瞰画面に案内「次の人（名前）は手を挙げてください」・人物一覧（検出中 / 割当済み / 見失い）・「割当をやり直す」を出し、スマホの案内も「俯瞰画面の案内に合わせて手を挙げて」にした。見失っている間もスマホは最後の位置で撃ち続けられる（08-3 と同じ）。ヘッドレスで「1 秒の隠れは残る・3 秒で外れて自動では戻らない・挙げると戻る・やり直し・再接続」を確かめた。サーバー側で割当を持つ（再接続で名前から引き継ぐ等）・見た目の特徴での再同定は**未実装**
- **SDK ならどう解決するか（案）**: 「現実の人物 ↔ プレイヤー」の対応づけ（Phase 9 の person-id と同じ問題）を SDK の一機能にして、対応の状態をサーバー（Room）に持たせ、再接続では名前や端末のトークンで引き継ぐ。対応の手がかりを「挙手の儀式」だけでなく、スマホ側の動き（ジャイロのヨー角速度と人物の向きの変化の相関、スマホの画面の点滅を PC のカメラで見る）で自動化する口を用意する。割当が外れた・入れ替わった可能性を HUD と俯瞰画面に出すところまで含めて共通化する
- **関連**: `src/shared/pose-cam-assign.ts`、`demos/08-8-splatoon-pose-cam/tracker.ts` の `applyPoses`、`demos/08-8-splatoon-pose-cam/overview.ts` の `trackerPlayers` / 人物一覧、PAIN_POINTS の Phase 9（09 person-id）の対応づけの項

## [2026-09-17] 08-8 Pose カメラ / server: メッセージが同じサーバーを別パスでもう 1 つ登録したいだけなのに、plugin のパスが定数で、httpServer の "upgrade" の購読を Proxy で包む回り道になった

- **何が苦しかったか**: 08-8 のサーバーは 08-3（`server/splatoon-outside-in.ts`）とメッセージも検証も同じで、違うのはパス（Room を 08-3 と分けたい）だけ。ところが `splatoonOutsideInServer()` は `roomServerPlugin` の spec にパス定数を埋め込んで plugin を返すだけで、spec 自体は export されていない。08-3 のファイルは変更しない方針なので、パスを引数にもできない。370 行をもう一度コピーするのは避けたかった
