...
3. **パーを開いている間、手からインクを連射**します（「目→手」の向き、約4発/秒）。インクタンクは満タンで40発（視界下のゲージ）。空になると撃てません。撃った直後1秒は回復せず、**自分の色の床の上にいるとゲージが速く回復**します（約2秒で満タン。それ以外は約8秒）。壁か床に当たった場所が自分の色になり、他人の色は上書きされます。
