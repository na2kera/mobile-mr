...
| [08. MR Splatoon](./demos/08-splatoon/) | 壁と床をフィールドに、グーでチャージ→パーで手からインクを発射。2チームで塗った面積を競う | スマートフォン1台以上、壁に貼ったマーカー |
...
1. マーカーを壁に天地を合わせて貼ります。マーカーを中心にした壁（既定 1.5m × 1.0m、`?wallW=` / `?wallH=`）と、その下の床（マーカーから床までの高さ `?floorDrop=`、既定 1.0m。壁から手前へ `?floorDepth=`、既定 1.5m）がフィールドです。マーカーの高さを床から測って `?floorDrop=` に入れると床の塗りが現実の床に乗ります。
2. 参加する全員が同じURLを開いて開始し、まず壁のマーカーを見ます（表示名は `?name=`、05で求めた `?handScale=` も付けてください）。参加順にオレンジ / ブルーへ交互に分かれ、最初の1人が入った時点で試合（既定90秒、`?matchSec=`）が始まります。
3. **グーを握るとチャージ**（視界内にゲージ。最大1.5秒）、**パーに開くと手からインクが発射**されます。インクは「目→手」の向きへ飛び、チャージが長いほど速く・大きく（半径5〜15cm）塗れます。壁か床に当たった場所がチーム色になり、相手の色は上書きされます。
4. 時間切れで塗った面積（壁 + 床のセル数）の多いチームの勝ち。結果表示のあと自動で次の試合が始まります。パーに開く動きで手を一瞬見失っても0.4秒は待ち、チャージが30%以上なら最後の手の位置から発射します。手が取れないときは、画面を押している間チャージ、離すと視界の中央へ発射（PCは Space キー）。
