...
| [08. MR Splatoon](./demos/08-splatoon/) | 壁と床をフィールドに、パーの間、手からインクを連射。全員が自分の色で塗り合う個人戦 | スマートフォン1台以上、壁に貼ったマーカー |
...
3. 自分の番になると視界内に「あなたの番です」と出ます。ボードから1.5〜2.4mほど離れて立ち、顔の前に出した手を前へ速く振ると、手のひらの速さと向きのままダーツが飛びます（狙いの補正はありません。速度は振りのピーク、位置は減速を検出した時点のものを使います）。自分の番の間に振り始めた動作だけが投げになります。他の人はその場から見ていてください（他人のダーツも自分の端末で飛んで刺さります）。
...
1. マーカーを壁に天地を合わせて貼ります。マーカーを中心にした壁（既定 3.0m × 2.4m、`?wallW=` / `?wallH=`）と、その下の床（マーカーから床までの高さ `?floorDrop=`、既定 1.2m。壁から手前へ `?floorDepth=`、既定 2.5m）がフィールドです。**マーカーの中心を床から 1.2m の高さに貼る**と、壁の下端と床がちょうど接続します（違う高さなら実測を `?floorDrop=` に）。
2. 参加する全員が同じURLを開いて開始し、まず壁のマーカーを見ます（表示名は `?name=`、05で求めた `?handScale=` も付けてください）。**個人戦**で、参加順に自分の色（8色）が割り当てられます。最初の1人が入った時点で試合（既定90秒、`?matchSec=`）が始まります。
3. **パーを開いている間、手からインクを連射**します（「目→手」の向き、約6発/秒）。インクタンクは満タンで50発（視界下のゲージ）。空になると撃てません。撃った直後1秒は回復せず、**自分の色の床の上にいるとゲージが速く回復**します（約2秒で満タン。それ以外は約8秒）。壁か床に当たった場所が自分の色になり、他人の色は上書きされます。
