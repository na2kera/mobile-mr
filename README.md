...
3. **パーを開いている間、手からインクを連射**します（「目→手」の向き、約6発/秒）。インクタンクは満タンで50発（手元に浮かぶ筒の水位で残量が見える。手が見えない間は視界の下）。空になると撃てません。**撃つのをやめると回復し、グーにしている間は速く回復**します（撃った直後1秒は回復せず、グーなら約2秒、それ以外は約5秒で満タン）。壁か床に当たった場所が自分の色の飛沫（いびつな本体 + 進行方向に飛び散る滴。壁では下に垂れる）になり、他人の色は上書きされます（塗り替えた瞬間は白く光り、着弾の音が鳴ります。`?sound=0` で無音）。飛沫の形は全端末とサーバーで同じで、得点もその形で数えます。
