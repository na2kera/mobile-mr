...
1. マーカーを壁に天地を合わせて貼ります。フィールドは床（既定 3.0m × 2.5m）を**四方の壁で囲んだ箱型のコート**です: 正面 = マーカーの壁（3.0m × 2.4m、`?wallW=` / `?wallH=`）、左右と背面は現実に壁が無くても立つ仮想の壁（高さ 2.4m。奥行きは `?floorDepth=`、既定 2.5m）。**マーカーの中心を床から 1.2m の高さに貼る**と、壁の下端と床がちょうど接続します（違う高さなら実測を `?floorDrop=` に）。
