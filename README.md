...
3. **パーを開いている間、手からインクを連射**します（「目→手」の向き、約4発/秒）。インクタンクは満タンで約15発（視界下のゲージ）。空になると撃てません。**自分の色の床の上にいるとゲージが速く回復**します（約3秒で満タン。それ以外は約20秒）。壁か床に当たった場所がチーム色になり、相手の色は上書きされます。
4. 時間切れで塗った面積（壁 + 床のセル数）の多いチームの勝ち。結果表示のあと自動で次の試合が始まります。手が取れないときは、画面を押している間、視界の中央へ連射（PCは Space キー）。
