...
| [08. MR Splatoon](./demos/08-splatoon/) | 四方の壁と床の箱型コートに、パーの間、手からインクを連射（グーで補充）。入室後は自由に塗れる練習、対戦（3分）の開始はPCの[俯瞰画面](./demos/08-splatoon/overview.html)から。全員が自分の色で塗り合う個人戦 | スマートフォン1台以上、壁に貼ったマーカー、対戦を始めるPC（俯瞰画面） |
...
3. **パーを開いている間、手からインクを連射**します（「目→手」の向き、約6発/秒）。インクタンクは満タンで50発（視界下のゲージ）。空になると撃てません。**撃つのをやめると回復し、グーにしている間は速く回復**します（撃った直後1秒は回復せず、グーなら約2秒、それ以外は約5秒で満タン）。壁か床に当たった場所が自分の色の飛沫（いびつな本体 + 進行方向に飛び散る滴。壁では下に垂れる）になり、他人の色は上書きされます（塗り替えた瞬間は白く光り、着弾の音が鳴ります。`?sound=0` で無音）。飛沫の形は全端末とサーバーで同じで、得点もその形で数えます。
4. 対戦はPCで [俯瞰画面](./demos/08-splatoon/overview.html)（`/demos/08-splatoon/overview.html`。カメラ・マーカー不要。スマホと同じ `?room=` で入る）を開き、必要なら「塗れる空間の大きさ」の幅・高さ・奥行きとマーカーの高さ（実測）を入れて「反映」（練習中か結果表示中だけ。カウントダウン中・対戦中は変えられません）、**「対戦開始」を押すと始まります**（カウントダウン `?waitSec=`、既定5秒 → 練習の塗りが消えて既定3分、`?matchSec=`）。俯瞰画面にはコート全体・全員の頭と手・塗り・得点・インク残量がリアルタイムに出ます（ドラッグで回転）。時間切れで塗った面積（壁 + 床のセル数）が最多の人の勝ち（壁の上のスコアボードに順位）。結果表示（8秒）のあと練習に戻り、次の対戦も俯瞰画面から始めます（結果表示中でも「次の対戦を開始」で始められます）。手が取れないときは、画面を押している間、視界の中央へ連射（PCは Space キー）。
