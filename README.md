...
ペイントの検証・順序・保持はViteに同居するサーバー（`server/surface.ts`。接続の共通部分は今回抽出した `server/room-server.ts`）が持ちます。マーカーとSurfaceの大きさはroom内で一致している必要があります。1 roomは8人まで、ペイントは1roomあたり4,000点まで保持し、超えると古い分から切り詰めて全員に配り直します（描き続けると1人で約4分半で到達。視界内に案内が出ます）。全員が切断してから10分でroomごと消えます（Wi-Fiの瞬断やSafariの画面切り替え程度では消えません）。全消去は2秒以内に2回押して確定です。
