...
3. **パーを開いている間、手からインクを連射**します（「目→手」の向き、約6発/秒）。インクタンクは満タンで50発（視界下のゲージ）。空になると撃てません。**撃つのをやめると回復し、グーにしている間は速く回復**します（撃った直後1秒は回復せず、グーなら約2秒、それ以外は約5秒で満タン）。壁か床に当たった場所が自分の色になり、他人の色は上書きされます。
