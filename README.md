...
1. マーカーを壁に天地を合わせて貼ります。フィールドは床（既定 3.0m × 2.5m）を**四方の壁で囲んだ箱型のコート**です: 正面 = マーカーの壁（既定 3.0m × 2.4m）、左右と背面は現実に壁が無くても立つ仮想の壁（奥行きは既定 2.5m）。壁の下端は常に床に接続します。**塗れる空間の大きさ（幅・高さ・奥行き）は俯瞰画面で開始前に変えられ**、サーバーが全員に配ります（URL では指定しません。変えると練習の塗りは消えます）。マーカーは壁の見やすい高さに貼り、**床からマーカー中心までの実測**を `?floorDrop=` に入れてください（既定 1.2m）。
