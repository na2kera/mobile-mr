...
2. 参加する全員が同じURLを開いて開始し、まず壁のマーカーを見ます（表示名は `?name=`、05で求めた `?handScale=` も付けてください）。**個人戦**で、参加順に自分の色（8色。試合中は退出者の色を使い回しません）が割り当てられます。入室したら**練習**で、時間無制限に自由に塗れます（視界に「パーで塗る ／ グーで補充」の案内が出ます）。
