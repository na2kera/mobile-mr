...
| [10. MR Golf](./demos/10-golf/) | 各ラウンド一人一打のニアピン勝負。全員の一打後は結果を保持し、PCの俯瞰画面から次の配置へ進める。カップは左右斜めに移り、角度と距離が変わる。カップイン優先・残り距離で順位点を付け、3ラウンドの合計点で勝負。視線＋タップ／Aで狙いを固定し、Joy-Conを振る（未設定の狙いは正面） | スマートフォン1台以上、壁と床に貼ったマーカー、Joy-Conを繋いだPC（[俯瞰画面](./demos/10-golf/overview.html)、Chrome。無ければ画面長押しで溜め打ち） |
