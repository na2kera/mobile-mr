...
4. 対戦はPCで [俯瞰画面](./demos/08-splatoon/overview.html)（`/demos/08-splatoon/overview.html`。カメラ・マーカー不要。スマホと同じ `?room=` とフィールド設定で入る）を開き、**「対戦開始」を押すと始まります**（カウントダウン `?waitSec=`、既定5秒 → 練習の塗りが消えて既定3分、`?matchSec=`）。俯瞰画面にはコート全体・全員の頭と手・塗り・得点・インク残量がリアルタイムに出ます（ドラッグで回転）。時間切れで塗った面積（壁 + 床のセル数）が最多の人の勝ち（壁の上のスコアボードに順位）。結果表示のあと練習に戻り、次の対戦も俯瞰画面から始めます。手が取れないときは、画面を押している間、視界の中央へ連射（PCは Space キー）。
