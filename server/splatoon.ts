...
// pose / shot / state を受け取り、唯一「start（対戦開始）」「stop（途中終了）」「dismiss（結果を閉じる）」と「field（フィールドの寸法の変更）」を送れる（issue #19 / #21 / #32 / #45）。
  ...
  return roomServerPlugin<SplatoonRoomConfig, State, ClientMessage>("splatoon-server", {
    ...
    onMessage(room: Ctx, id, msg, now) {
      ...
      if (msg.type === "stop") {
        ...
        if (events.length === 0) {
          console.log(`[splatoon] ${id} stop rejected: ${game.lastRejectReason}`);
          room.send(id, { type: "rejected", reason: game.lastRejectReason } satisfies ServerMessage);
          return;
        }
