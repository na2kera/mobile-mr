...
import { TRACK_STALE_MS } from "../../src/shared/splatoon-outside-in-protocol";
import type { PlayerPose, TrackEntry, TrackedPlayer, TrackerStatus } from "../../src/shared/splatoon-outside-in-protocol";
import { parseFakePlayersParam, parseFakeTrackCamParam } from "../../src/shared/outside-in-track";
...
// 08-3 アウトサイドイン版の俯瞰画面。08 の overview.ts のコピーに「トラッカー」を足したもの（docs/space-stability-options.md §4 の 2a）:
//   - パネルの「トラッカー」で PC の Web カメラを開くと、tracker.ts がゴーグル前面のマーカー（ID = 入室順の割当。一覧に出す）を検出し、
//     部屋（field）座標系での位置とヨーを 2 本目の接続（?role=tracker）でサーバーへ送る（track）。サーバーがプレイヤーに紐づけて全員に配る
//   - 原点合わせ: 原点マーカー（正面の壁。room の markerId）か「追加マーカーの配置」のマーカー（床など）をカメラで見て「原点を確定」。
//     確定後は原点マーカーが隠れてもよい（カメラを動かしたら確定し直す）。カメラは壁側から部屋を向けて置くと、正面を向いた
//     ゴーグルのマーカーが大きく映る（原点は床のマーカーで合わせる）
//   - 一覧の「位置合わせ」はマーカーの ID ではなく「トラッカー ID n q=品質 / 見失い」を出す（tracked から）
//   - PC 確認: ?tracker=1 で入室後にカメラを自動で開く。?fakecam=1 でフェイクカメラ（?fakePlayers= のゴーグルを部屋に置く）
// 以下は 08 の説明:
...
/** ゴーグルに貼るマーカーの一辺 [mm]（原点・追加マーカーは ?markerMm=） */
const TRACK_MARKER_MM = numParam("trackMarkerMm", 60, { min: 10, max: 1000 });
...
/** フェイクカメラのゴーグル: ?fakePlayers=10:-0.5,0,1.5,0;11:0.5,0,1.5,15（ID:x,y,z,ヨー[deg]） */
const FAKE_PLAYERS = parseFakePlayersParam(params.get("fakePlayers"));
...
type Peer = {
  ...
  lastPoseMs: number;
  tracking: boolean;
  /** ゴーグルに貼るマーカーの ID（welcome / join の割当） */
  trackId: number | null;
  /** トラッカーの直近の観測（tracked） */
...
async function startTracker() {
  ...
  tracker = createTracker({
    ...
    goggleSizeM: TRACK_MARKER_MM / 1000,
    camHFovDeg: TRACK_CAM_FOV,
    detW: TRACK_DET_W,
    intervalMs: TRACK_INTERVAL_MS,
    maxPoseError: TRACK_MAX_POSE_ERROR,
    eyeBackM: TRACK_EYE_BACK_M,
    qualityFullPx: TRACK_QUALITY_PX,
  ...
  const ranking = s
    ? [...s.players]
        ...
        .map((p) => {
          ...
          // 08 の「マーカー n」の代わりにトラッカーの状態（割当の ID・品質・見失い）
          const marker = !peer
            ? "-"
            : !peer.tracked
              ? `トラッカー待ち（ID ${peer.trackId ?? "?"}）`
              : now - peer.tracked.atMs > TRACK_STALE_MS
                ? `見失い（ID ${peer.trackId ?? "?"}・${((now - peer.tracked.atMs) / 1000).toFixed(1)}s。最後の位置を維持）`
                : `トラッカー ID ${peer.trackId ?? "?"} q=${peer.tracked.quality.toFixed(2)}`;
          return { p, pct: (((s.scores[p.id] ?? 0) / total) * 100).toFixed(1), ink: s.ink[p.id] ?? 1, win: s.winners?.includes(p.id), marker };
        })
    : [];
  const key = JSON.stringify([phaseText, canStart, startPending, canStop, stopText, canReset, resetPending, ranking, netStatus, lastRejectReason, peers.size, sizeEditable, canApplySize, fieldPending, sizeHintText, canEditMarkers, canApplyMarkers, markersPending, markersHintText, rowStates, trackerStartText, canLock, trackerLockText, trackerText]);
