...
- **スマホ**（`main.ts`）: マーカーを検出しない（`createMarkerAnchor` を使わない）。アンカー（field → ワールド）は「Y 回転 φ + 並進」だけ:
  ...
  首を振っている間（カメラのヨー角速度が `?yawMaxRate=` を超える）と最後の発射から 300ms 以内（08 の `canSnap` と同じ）は緩やかな補正を止める。
  見失い明け・再接続・原点の確定し直しでは、位置と一緒に φ もスナップし直す（見失い明けのスナップは連射中なら 300ms 空くまで保留）。
...
- **サーバー**（`server/splatoon-outside-in.ts`、プロトコル `src/shared/splatoon-outside-in-protocol.ts` v2。08 の `server/splatoon.ts` は触らない）:
  役割 `player` / `overview` / `tracker`（`?role=tracker`）。プレイヤーの入室順にマーカー ID（`TRACK_ID_FIRST` = 10 から空いている番号。原点マーカーと追加マーカーの ID は避ける）を割り当てて `welcome.trackId` / `join.trackId` で伝える。
  `track`（tracker → server。`{ locked, players: [{ id, pos, yaw, quality }] }`）をマーカー ID → プレイヤーに引き当てて `tracked` で全員に配る。
  `pose` の `pos` は常にトラッカーの測定値（見失っていても最後の値）で上書きして配り、発射位置の検証（頭から 1.2m 以内）もその値で行う。自己申告の位置は使わない: 一度も測定されていないプレイヤーの `pose` は配らず、発射は `rejected: "not tracked yet"`（スマホは位置が届くまで撃てない旨を案内に出す）。トラッカーは 1 台だけ有効（最初に接続した tracker が所有者。2 台目は `rejected: "not active tracker"` で待機し、所有者が切断したら引き継ぐ。原点は未確定から）。
  妥当性検証（値の範囲・レート制限 40 回/s）は 08 に倣う。追加マーカーの配置は、ゴーグル用の ID（`TRACK_ID_FIRST` 以上・割当済み）と重なるものを `rejected` で拒否する（トラッカーが原点候補として消費してしまうため）。試合のルール（`splatoon-game.ts`）はそのまま
...
| スマホ | `yawMaxRate=30` | カメラのヨー角速度 [deg/s] がこれを超える間はヨーの緩やかな補正を止める（観測時刻と受信時刻のずれで偏るため） |
...
| 俯瞰 | `trackQualityPx=30` | 品質が 1 になる画面上の辺長 [px]。既定（1280x720・`camFov=68` で焦点距離 ≈ 949px、60mm のマーカー）では品質 ≈ 0.76（2.5m）/ 0.54（3.5m）なので、`yawQuality=0.5` と合わせて**約 3.5m 以内の正対でヨーが補正される** |
...
ヘッドレス（`scripts/headless-08-3-outside-in.mjs`）が見ていること: トラッカー待ち → 原点未確定 → 位置待ち の順に案内が変わり撃てない / 床のマーカーで原点を確定 / 両方の位置（5cm 以内）とヨー（2° 以内）が届き、自分の正面を field に直すとトラッカーのヨーと一致（φ が効く）/ 相手が正しい相対位置（5cm 以内）に見える / 発射が着弾して得点 / 俯瞰画面の一覧が「トラッカー ID 10 / 11」/ ゴーグルを隠すと最後の位置を保持して `tracked (3.0s ago)` を出し撃ち続けられ、再び見えれば復帰 / 対戦開始 → 終了 → 結果を閉じる / 原点を確定し直すとずらしたヨーがスナップして戻る / 再接続すると位置が届くまで案内「届くまで撃てません」が出て撃たず、見えれば追跡に戻る。
...
- **カメラの視野内でしか遊べない**（水平 FOV 68° のカメラで 3m 幅のコートは 2.5m 以上離す）。視野の外・他の人に隠れた間は最後の位置を保持する（2 台目のカメラは未対応: `MAX_TRACKERS` = 2 で接続は受けるが**有効なトラッカーは 1 台**で、2 台目の `track` は `not active tracker` で拒否され、所有者が切断したときの予備になる）
...
- **トラッカーの所有者の PC が close を送れずに落ちたとき**（電源断・ネットワーク断）、予備のトラッカーへの引き継ぎと本人の再接続は heartbeat の 2 周期（約 20 秒）待つ。その間プレイヤーは最後の位置で撃ち続けられる
