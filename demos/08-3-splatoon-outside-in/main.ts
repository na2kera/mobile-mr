...
import { NAME_MAX_LENGTH, TRACK_STALE_MS } from "../../src/shared/splatoon-outside-in-protocol";
import type { PlayerPose, TrackedPlayer, TrackerStatus } from "../../src/shared/splatoon-outside-in-protocol";
...
// 08-3: MR スプラトゥーン「アウトサイドイン」版（docs/space-stability-options.md §4 の 2a）。
// 08（demos/08-splatoon/main.ts）を丸ごとコピーして「位置の取り方」だけ差し替えたもの。ゲームの仕様・UI・HUD は 08 のまま。
//   - 位置の取り方（08 との差分）: スマホはマーカーを検出しない（createMarkerAnchor を使わない）。部屋に固定した PC の
//     Web カメラ（俯瞰画面のトラッカー。overview.html）がゴーグル前面のマーカー（ID = 入室順に割り当て。welcome の trackId）を
//     検出して部屋（field）座標系での位置とヨーをサーバーへ送り、サーバーが全員に配る（tracked）。スマホは自分の位置を
//     そこから受け取り、回転はジャイロ（DeviceOrientationControls）のまま。アンカー（field → ワールド）は
//     「Y 回転 φ + 並進」だけ: φ = ワールドでのカメラのヨー − トラッカーのヨー（正面を向いてマーカーが大きく映った =
...
//   - マルチマーカー（issue #30）の配置は 08 と同じく俯瞰画面から配られるが、08-3 のスマホは検出には使わない
...
// アウトサイドイン固有（08 の detW / smooth / markerIntervalMs / lostMs / maxPoseError の代わり）
...
/** ヨーの補正: 品質がこれ以上の観測だけ使う（0..1。正面を向いてマーカーが大きく映ったとき） */
...
// 位置はトラッカー（俯瞰画面の ?fakecam=1&fakePlayers=）から届く）
...
// 08-3 のスマホはマーカーを検出しないが、枠が実物のマーカーに重なるかで「トラッカーの原点合わせが合っているか」を目で確かめられる
...
// 08-3 のスマホはマーカーを検出しないので、フェイクカメラはチェッカーボード（動くバー）だけ。合成の手（?fakehands=1）はこれに重ねる
...
/** 自分のゴーグルに貼るマーカーの ID（welcome の trackId。HUD と案内に出す） */
  ...
  client = connectGame(
    ...
    },
    {
      ...
      onWelcome: (id, _role, peerIds, cfg, state, info) => {
        ...
        joined = true;
        posesSent = 0;
        // 再接続すると id が変わり、マーカーの割当も変わり得る（HUD と案内に出す。ゴーグルのマーカーを貼り替える）
        ...
        splatted.clear();
        onState(state);
        console.log(`[game] joined "${ROOM}" as ${id} color=${myColor} marker=${myTrackId} tracker=${info.tracker.connected}/${info.tracker.locked ? "locked" : "unlocked"} (peers: ${peerIds.join(", ") || "none"})`);
      },
      onPeerJoin: (id, trackId) => {
        createPeer(id);
        console.log(`[game] peer ${id} joined (marker ${trackId})`);
      },
      onPeerLeave: (id) => {
        removePeer(id);
...
// ヘッドレス確認用（scripts/headless-08-3-outside-in.mjs）: 自分の field 座標系での位置・向き、ピアの位置
const dbgForward = new THREE.Vector3();
const dbgQuat = new THREE.Quaternion();
(window as Window & { __outsideIn?: unknown }).__outsideIn = {
  /** 補間中の自分の位置（field 座標系） */
