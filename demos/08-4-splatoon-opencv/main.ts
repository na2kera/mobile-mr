...
import { createOpenCvMarkerAnchor } from "../../src/shared/marker-anchor-opencv";
import type { OpenCvMarkerAnchor, PoseSource } from "../../src/shared/marker-anchor-opencv";
...
// 08-4: MR スプラトゥーン（OpenCV.js の ArUco board + solvePnP 版）。demos/08-splatoon の丸ごとコピーで、
// 変えたのは「位置の取り方（自分のカメラの姿勢を field 座標系でどう決めるか）」だけ（docs/space-stability-options.md §4 の 1b）。
//   - 08: js-aruco2 で検出 → 1 枚ずつ POSIT で姿勢 → 重み付き平均（src/shared/marker-anchor.ts）
//   - 08-4: OpenCV.js（WASM）の ArucoDetector で検出（辞書は同じ ARUCO_MIP_36h12。コーナーはサブピクセル精錬）→
//     見えている全マーカー（原点 + 俯瞰画面で配った追加マーカー）の 4 隅を 1 つの board にして solvePnP で 1 つの姿勢
//     （src/shared/marker-anchor-opencv.ts / opencv-pose.ts / pnp-board.ts）。重力での水平化（gravityAlign）は同じ考え方で残す
//   - OpenCV.js は `npm run fetch:opencv` で public/vendor/opencv/ に置く（実行時に fetch して動的 import）。無い・失敗なら
...
// 08-4 固有: 位置の取り方（先頭のコメント参照）
const detectorRaw = (params.get("detector") ?? "opencv").toLowerCase();
const DETECTOR: "opencv" | "aruco2" = detectorRaw === "aruco2" ? "aruco2" : "opencv";
...
let markerAnchor: OpenCvMarkerAnchor | null = null;
...
async function startCameraAndMarker(onProgress: (step: string) => void) {
  ...
  markerAnchor = createOpenCvMarkerAnchor({
    opencvUrl: OPENCV_URL,
    detector: DETECTOR,
    pnpMethod: PNP_METHOD,
    poseSource: POSE_SOURCE,
    maxReprojPx: MAX_REPROJ_PX,
    video: pt.video,
    camera,
    anchor,
...
function renderHud() {
  ...
  const text = [
    ...
    `pose=${markerAnchor?.backend ?? "-"} ${markerAnchor?.info ?? "-"}${markerAnchor?.everDetected && !markerAnchor.isTracking(now, MARKER_LOST_MS) ? " (holding last pose)" : ""}${markerAnchor?.backend === "aruco2" && markerAnchor.loadStatus !== "detector=aruco2" ? ` (${markerAnchor.loadStatus})` : ""} layout=${describeMarkers(fieldCfg.markers ?? [])} self=${lastSelfPos ? `(${lastSelfPos.map((v) => v.toFixed(2)).join(",")})` : "-"}`,
...
nameForm.addEventListener("submit", (event) => {
  ...
  hudState.base = `fov=${FOV_FIXED ?? "auto"} camZoom=${CAM_ZOOM} markerMm=${MARKER_MM} detW=${MARKER_DET_W}@${MARKER_INTERVAL_MS}ms gravityAlign=${GRAVITY_ALIGN ? 1 : 0} detector=${DETECTOR} pnp=${PNP_METHOD} poseSource=${POSE_SOURCE} hands=${NUM_HANDS} delegate=${DELEGATE} handScale=${HAND_SCALE} gravity=${GRAVITY} matchSec=${MATCH_SEC} mode=${touch ? "gyro" : "orbit"}`;
  connect(name);
  if (FAKE_HANDS) {
