...
import { createDetectorAnchor } from "./marker-anchor-apriltag";
import { createAprilTagDetector, loadAprilTagWasm } from "./apriltag-detector";
import { fakeAprilTag, drawFakeAprilTags } from "./fake-apriltags";
...
import { fakeCameraToField, parseFakeMarkersParam } from "../../src/shared/fake-markers";
...
// 08-5: MR スプラトゥーン（AprilTag 版）。docs/space-stability-options.md §4 の候補 1c「検出器を替える」。
// 08（demos/08-splatoon）を丸ごとコピーし、「位置の取り方」のうち **マーカーの検出器だけ** を js-aruco2（ARUCO_MIP_36h12 +
// POSIT）から AprilTag（tag36h11 の WASM。apriltag C ライブラリの検出 + estimate_tag_pose）に替えたもの。
// ゲームの仕様・UI・サーバー（server/splatoon.ts、同じ SPLATOON_PATH。room 名で分かれる）・重力での水平化（gravityAlign）・
// 複数マーカーの重み付き平均（fusePoseCandidates）は 08 と同じ。08 との差分はこのファイルの「検出器」まわりと、
// apriltag-detector.ts / marker-anchor-apriltag.ts / apriltag-pose.ts / tag36h11.ts / fake-apriltags.ts / markers.ts。詳細は README.md。
//   - マーカーは tag36h11（絵柄が ArUco と違う。markers.html で印刷。ID の対応は 08 と同じ: 原点 ID 0、追加マーカーは俯瞰画面の ID）
//   - WASM は public/vendor/apriltag/（npm run fetch:apriltag）。読めなければ HUD に出して js-aruco2 に落ちる（?detector=aruco2 と同じ経路）
//   - 姿勢は AprilTag の R, t（カメラ座標系 X 右・Y 下・Z 前）を apriltag-pose.ts で 08 と同じ field 座標系（マーカー面 = Z=0、Y 上）に直す。
//     焦点距離 fx = fy は 08 と同じく camHFovDeg（?camFov= で上書き）から、主点は検出画像の中心
//   - 固有の URL パラメータ: ?detector=apriltag|aruco2（既定 apriltag）、?aprilDecimate=2（quad_decimate。1 で全画素）、?aprilSigma=0
//   - HUD の marker= の行は april=（AprilTag のとき。末尾の ms が検出 1 回の処理時間）。aruco2 に落ちたときは marker= のまま
//
// 以下は 08 のコメントのまま。
...
/** 検出器: apriltag（既定。tag36h11 の WASM）か aruco2（08 と同じ js-aruco2。比較用）。WASM が読めなければ aruco2 に落ちる */
const DETECTOR: "apriltag" | "aruco2" = params.get("detector") === "aruco2" ? "aruco2" : "apriltag";
/** AprilTag の quad_decimate（入力を 1/n に間引いて四角形を探す。角は refine_edges で全解像度に戻る。同梱の例の既定 2.0。1 で全画素） */
const APRIL_DECIMATE = numParam("aprilDecimate", 2, { min: 1, max: 8 });
/** AprilTag の quad_sigma（分割前のガウスぼかし [px]。既定 0） */
const APRIL_SIGMA = numParam("aprilSigma", 0, { min: 0, max: 5 });
/** apriltag_wasm.js / .wasm の置き場所（scripts/fetch-apriltag.mjs） */
const APRIL_BASE_URL = `${import.meta.env.BASE_URL}vendor/apriltag/`;
...
/** 実際に使っている検出器（DETECTOR が apriltag でも WASM が読めなければ aruco2） */
let detectorActive: "apriltag" | "aruco2" = DETECTOR;
/** AprilTag の WASM の状態（HUD 用）: idle / loading / ready / error: ... / unused（?detector=aruco2） */
let aprilStatus = DETECTOR === "apriltag" ? "idle" : "unused";
...
function fakeWorld(): { markers: FakeMarker[]; camToField: number[] } {
  ...
  // 絵柄は実際に使う検出器に合わせる（AprilTag なら tag36h11、aruco2 に落ちたら ARUCO_MIP_36h12）。格子（10 セル）は同じ
  const fake = (id: number, toField: number[]): FakeMarker => (detectorActive === "apriltag" ? fakeAprilTag(id, toField) : { id, bits: markerBits(id), toField });
  const markers: FakeMarker[] = [fake(MARKER_ID, originToField)];
  for (const m of FAKE_MARKERS) {
    if (m.id === MARKER_ID || !(MARKER_FACES as readonly string[]).includes(m.face)) continue;
    markers.push(fake(m.id, markerToFieldMatrix({ id: m.id, face: m.face as MarkerFace, pos: m.pos })));
  }
...
function fakeStream(): MediaStream {
  ...
  return createFakeCameraStream(
    (ctx, canvas, frame) => {
      ...
      drawFakeAprilTags(ctx, visible, world.camToField, FAKE_FOCAL_PX, canvas.width, canvas.height, MARKER_SIZE_M);
    },
...
async function startCameraAndMarker(onProgress: (step: string) => void) {
  ...
  if (DETECTOR === "apriltag") {
    ...
    try {
      ...
      aprilStatus = `error: ${msg}`;
      detectorActive = "aruco2";
      console.warn(`[apriltag] WASM を読めないので js-aruco2 に落とす: ${msg}`);
    }
  }
  if (!detector) detector = createMarkerDetector(MARKER_SIZE_M);
...
function renderHud() {
  ...
  const text = [
    ...
    `${detectorActive === "apriltag" ? "april" : "marker"}=${markerAnchor?.info ?? "-"}${markerAnchor?.everDetected && !markerAnchor.isTracking(now, MARKER_LOST_MS) ? " (holding last pose)" : ""} layout=${describeMarkers(fieldCfg.markers ?? [])} self=${lastSelfPos ? `(${lastSelfPos.map((v) => v.toFixed(2)).join(",")})` : "-"}`,
