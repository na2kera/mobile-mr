...
import { TRACK_STALE_MS } from "../../src/shared/splatoon-pose-cam-protocol";
import type { PlayerPose, TrackEntry, TrackedPlayer, TrackerStatus } from "../../src/shared/splatoon-pose-cam-protocol";
import { parseFakeTrackCamParam } from "../../src/shared/outside-in-track";
import { parseFakeBodiesParam } from "../../src/shared/pose-cam-track";
import { createTracker } from "./tracker";
import type { Tracker } from "./tracker";
import { CAM_FOV_WIDE } from "../../src/shared/passthrough-camera";
...
// 08-8（PC カメラ + MediaPipe Pose で人を追う。docs/space-stability-options.md §4 の 2b）版の俯瞰画面。
// 08-3（アウトサイドイン）の overview.ts のコピーで、トラッカーを「ゴーグルのマーカー」から「人物の検出」に差し替えたもの:
//   - パネルの「トラッカー」で PC の Web カメラを開くと、tracker.ts が PoseLandmarker で人物を検出し、各人の頭の位置
...
// ---- トラッカー（tracker.ts）のパラメータ ----
/** 入室したらカメラを自動で開く（ヘッドレス確認用。実運用はボタン） */
const TRACKER_AUTO = params.get("tracker") === "1";
/** Web カメラの水平 FOV [deg]（既定は標準レンズの推定値。実測できるなら合わせる: 距離が定数倍ずれる） */
const TRACK_CAM_FOV = numParam("camFov", CAM_FOV_WIDE, { min: 10, max: 170 });
/** 原点マーカーの検出の画像の長辺 [px]（原点が未確定の間だけ検出する） */
const TRACK_DET_W = numParam("trackDetW", 1280, { min: 64, max: 4096 });
/** 検出（原点マーカー / 人物）の間隔 [ms] */
const TRACK_INTERVAL_MS = numParam("trackIntervalMs", 66, { min: 16, max: 2000 });
const TRACK_MAX_POSE_ERROR = numParam("maxPoseError", 0.5, { min: 0, max: 100 });
...
const NECK_M = numParam("neckM", 0.25, { min: 0, max: 0.6 });
/** 頭の中心からスマホのカメラ（ゴーグルの前面）までの距離 [m]（正面へ） */
const EYE_FWD_M = numParam("eyeFwdM", 0.1, { min: 0, max: 0.4 });
/** 点を使う可視性（visibility）の下限 */
...
const OFFICIAL_POSE_MODEL_URL = "https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_lite/float16/1/pose_landmarker_lite.task";
const POSE_MODEL_URLS = params.get("poseModel") ? [params.get("poseModel")!] : [`${import.meta.env.BASE_URL}models/pose_landmarker_lite.task`, OFFICIAL_POSE_MODEL_URL];
const FAKE_CAM = params.has("fakecam");
/** フェイクカメラの合成の体: ?fakeBodies=-0.5,0,1.5,0;0.5,0,1.5,15（頭の中心 x,y,z [m]・ヨー [deg]） */
const FAKE_BODIES = parseFakeBodiesParam(params.get("fakeBodies"));
/** フェイクカメラの映像にも本物の PoseLandmarker を回す（モデルの読み込みと推論の経路の確認用。観測は合成の体のまま） */
const FAKE_POSE_MODEL = params.get("fakePoseModel") === "1";
/** フェイクカメラの合成の体がゴーグルで目を隠しているか（?fakeGoggles=0 で目も見える） */
const FAKE_GOGGLES = params.get("fakeGoggles") !== "0";
/** フェイクカメラの姿勢: ?fakeTrackCam=x,y,z,yaw,pitch（既定は正面の壁の原点マーカーの 0.5m 上・0.2m 手前から部屋を 35° 見下ろす） */
const FAKE_TRACK_CAM = parseFakeTrackCamParam(params.get("fakeTrackCam"), { pos: [0, 0.5, 0.2], yawDeg: 180, pitchDeg: 35 });
...
type Peer = {
  ...
  lastPoseMs: number;
  tracking: boolean;
  /** サーバーが入室順に付けた番号（welcome / join の trackId。トラッカーはこの番号で track を送る） */
  trackId: number | null;
  /** トラッカーの直近の観測（tracked） */
  tracked: { pos: V3; quality: number; atMs: number } | null;
};
const peers = new Map<string, Peer>();
const headGeometry = new THREE.SphereGeometry(0.12, 24, 16);
const noseGeometry = new THREE.ConeGeometry(0.05, 0.14, 16);

function createPeer(id: string, trackId: number | null = null): Peer {
  removePeer(id);
...
let markersSent = 0;
/** サーバーのトラッカーの状態（welcome / tracked） */
let trackerStatus: TrackerStatus = { connected: 0, locked: false };
...
function onTracked(status: TrackerStatus, players: TrackedPlayer[]) {
  ...
  for (const t of players) {
    const peer = peers.get(t.player);
    if (!peer) continue;
    peer.trackId = t.id;
    peer.tracked = { pos: t.pos, quality: t.quality, atMs: now };
  }
}
...
function connect() {
  ...
  client = connectGame(
    ...
    },
    {
      ...
      onWelcome: (id, role, peerIds, cfg, state, info) => {
        selfId = id;
        netStatus = "open";
        joined = true;
        trackerStatus = info.tracker;
        applyFieldConfig(cfg);
        syncSizeInputs();
        syncMarkerRows();
        [...peers.keys()].forEach(removePeer);
        peerIds.forEach((pid) => createPeer(pid, info.trackIds[pid] ?? null));
        if (TRACKER_AUTO && !tracker) void startTracker();
        lastEventKey = "";
        for (const s of shots.values()) s.mesh.removeFromParent();
        shots.clear();
        splatted.clear();
      ...
      onPeerJoin: (id, trackId) => {
        createPeer(id, trackId);
        renderPanel();
      },
      onPeerLeave: (id) => {
        removePeer(id);
...
// ---- トラッカー（tracker.ts）: Web カメラを開き、検出を 2 本目の接続（?role=tracker）で送る ----
const trackerStartButton = document.querySelector<HTMLButtonElement>("#tracker-start")!;
const trackerLockButton = document.querySelector<HTMLButtonElement>("#tracker-lock")!;
const trackerStatusEl = document.querySelector<HTMLDivElement>("#tracker-status")!;
const trackerPreviewEl = document.querySelector<HTMLDivElement>("#tracker-preview")!;
const trackerResetButton = document.querySelector<HTMLButtonElement>("#tracker-reset")!;
const trackerPromptEl = document.querySelector<HTMLDivElement>("#tracker-prompt")!;
const trackerPersonsEl = document.querySelector<HTMLUListElement>("#tracker-persons")!;
...
function trackerPlayers(): { id: string; trackId: number }[] {
  return [...peers].filter(([, p]) => p.trackId !== null).map(([id, p]) => ({ id, trackId: p.trackId! }));
}
...
function connectTracker() {
  if (ROOM === null || trackerClient) return;
  const ignore = () => {};
  trackerClient = connectGame(
    ROOM,
    "",
    { markerId: MARKER_ID, markerMm: MARKER_MM, gravity: GRAVITY, matchSec: MATCH_SEC, waitSec: WAIT_SEC },
    {
      onStatus: (status) => {
        trackerNet = status;
        renderPanel();
      ...
      onError: (reason) => {
        trackerNet = `error: ${reason}`;
        console.warn(`[tracker] rejected: ${reason}`);
        renderPanel();
      },
      onWelcome: (id, role) => {
        trackerNet = "open";
        console.log(`[tracker] joined "${ROOM}" as ${id} (${role})`);
      },
      onPeerJoin: ignore,
      onPeerLeave: ignore,
      onPeerPose: ignore,
...
async function startTracker() {
  ...
  tracker = createTracker({
    originId: MARKER_ID,
    markerSizeM: MARKER_MM / 1000,
    camHFovDeg: TRACK_CAM_FOV,
    detW: TRACK_DET_W,
    intervalMs: TRACK_INTERVAL_MS,
    maxPoseError: TRACK_MAX_POSE_ERROR,
    camRes: TRACK_CAM_RES,
    camLabel: TRACK_CAM_LABEL,
    extraMarkers: () => fieldCfg.markers ?? [],
    lockSamples: TRACK_LOCK_SAMPLES,
    numPoses: POSES,
    delegate: POSE_DELEGATE,
    modelUrls: POSE_MODEL_URLS,
    poseDetW: POSE_DET_W,
    shoulderM: SHOULDER_M,
    neckM: NECK_M,
    eyeFwdM: EYE_FWD_M,
    minVisibility: POSE_MIN_VIS,
    qualityFullPx: POSE_QUALITY_PX,
    matchM: ASSIGN_MATCH_M,
    lostMs: ASSIGN_LOST_MS,
    ...
    onAssign: (player, key) => {
      assignsDone++;
      console.log(`[overview] ${playerName(player)} (${player}) → person #${key}`);
      renderPanel();
    },
    onChange: renderPanel,
  });
  trackerPreviewEl.replaceChildren(tracker.preview);
  connectTracker();
  try {
    await tracker.start();
  } catch {
    // 状態は tracker.error / info に入っている（renderPanel が出す）
  }
  renderPanel();
}

trackerStartButton.addEventListener("click", () => {
  if (!tracker) void startTracker();
  else if (tracker.running) tracker.stop();
  else void tracker.start();
});
trackerResetButton.addEventListener("click", () => {
  tracker?.resetAssignments();
  renderPanel();
});
trackerLockButton.addEventListener("click", () => {
  if (!tracker) return;
  if (tracker.locked) tracker.unlock();
  else if (!tracker.lock()) lastRejectReason = "原点マーカーが見えていません";
  renderPanel();
});
...
function renderPanel() {
  ...
  // トラッカーの表示（カメラの状態・原点の確定・観測）
  const trackerStartText = !tracker ? "カメラを開く" : tracker.running ? "カメラを止める" : "カメラを開く";
  const canLock = tracker !== null && tracker.running && (tracker.locked || tracker.originVisible(now));
  const trackerLockText = tracker?.locked ? "原点を解除" : "原点を確定";
  ...
  // 割当の儀式: 入室順で次のプレイヤーに手を挙げてもらう
  const nextPlayer = tracker?.running && tracker.locked ? tracker.nextPlayer() : null;
  const promptText =
    !tracker?.running || !tracker.locked
      ? ""
      : peers.size === 0
        ? "プレイヤーの入室を待っています"
        : nextPlayer
  ...
  const ranking = s
    ? [...s.players]
        ...
        .map((p) => {
          ...
          // 08 の「マーカー n」の代わりにトラッカーの状態（割当・品質・見失い）
          const marker = !peer
            ? "-"
            : !peer.tracked
              ? nextPlayer === p.id
                ? "割当待ち（手を挙げる番）"
                : "割当待ち"
              : now - peer.tracked.atMs > TRACK_STALE_MS
    ...
    : [];
  const key = JSON.stringify([phaseText, canStart, startPending, canStop, stopText, canReset, resetPending, ranking, netStatus, lastRejectReason, peers.size, sizeEditable, canApplySize, fieldPending, sizeHintText, canEditMarkers, canApplyMarkers, markersPending, markersHintText, rowStates, trackerStartText, canLock, trackerLockText, trackerText, promptText, personRows, canResetAssign]);
  ...
  trackerStartButton.disabled = !joined;
  trackerStartButton.textContent = trackerStartText;
  trackerLockButton.disabled = !canLock;
  trackerLockButton.textContent = trackerLockText;
  trackerStatusEl.textContent = trackerText;
  trackerPromptEl.textContent = promptText;
  trackerResetButton.disabled = !canResetAssign;
  trackerPersonsEl.replaceChildren(
    ...personRows.map((row) => {
  ...
  playersEl.replaceChildren(
    ...ranking.map(({ p, pct, ink, win, marker }) => {
      ...
      markerEl.textContent = `位置合わせ: ${marker}`;
...
function renderHud() {
  ...
  const now = performance.now();
  const text = s
    ? `overview: room=${ROOM} me=${selfId} ws=${netStatus} phase=${s.phase} left=${remainingSec(now).toFixed(0)}s players=${s.players.map((p) => `${p.id}:${p.color}`).join(",")} scores=${s.players.map((p) => `${p.id}:${s.scores[p.id] ?? 0}`).join(",")} total=${s.totalCells} field=${fieldCfg.wallW}x${fieldCfg.wallH}x${fieldCfg.floorDepth}/${fieldCfg.floorDrop} markers=${describeMarkers(fieldCfg.markers ?? [])} live=${shots.size} seq=${s.seq} starts=${startsSent} stops=${stopsSent} resets=${resetsSent} dismisses=${dismissesSent} fields=${fieldsSent} markersSent=${markersSent} markerDrags=${markerDrags} peerTrack=${[...peers].map(([id, p]) => `${id}:${p.trackId ?? "?"}:${!p.tracked ? "wait" : now - p.tracked.atMs > TRACK_STALE_MS ? "lost" : "ok"}`).join(",")} tracker=${trackerStatus.connected}/${trackerStatus.locked ? "locked" : "unlocked"} tracked=${trackedCount} local=${tracker ? `${tracker.running ? "on" : "off"}/${tracker.locked ? "locked" : "unlocked"}/origin=${tracker.lastOriginIds.join("+") || "-"}/sent=${tracksSent}/players=${tracker.lastEntries.map((e) => `${e.player}:${e.id}:(${e.pos.map((v) => v.toFixed(2)).join(",")}):${e.quality.toFixed(2)}`).join(";") || "-"}` : "-"} persons=${tracker ? tracker.persons.map((t) => `${t.key}:${t.player ?? "-"}:${t.det.raised ? "up" : "down"}`).join(",") || "-" : "-"} next=${tracker?.running && tracker.locked ? (tracker.nextPlayer() ?? "none") : "-"} assigns=${assignsDone}`
