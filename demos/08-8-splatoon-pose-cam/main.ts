...
import { anchorPose, anchorYaw, approachAngle, wrapAngle, yawOfForward } from "../../src/shared/outside-in-track";
...
import { NAME_MAX_LENGTH, TRACK_STALE_MS } from "../../src/shared/splatoon-pose-cam-protocol";
import type { PlayerPose, TrackedPlayer, TrackerStatus } from "../../src/shared/splatoon-pose-cam-protocol";
...
// 08-8: MR スプラトゥーン「PC カメラ + MediaPipe Pose で人を追う」版（docs/space-stability-options.md §4 の 2b）。
// 08-3（アウトサイドイン。demos/08-3-splatoon-outside-in/main.ts）を丸ごとコピーしたもので、スマホ側の位置の取り方は 08-3 と同じ。
// 08-3 との違いはトラッカー（俯瞰画面の overview.ts / tracker.ts）だけで、ゴーグルにマーカーを貼らず、PC の Web カメラの映像から
// PoseLandmarker で人物を検出して頭の位置を出す。誰がどのプレイヤーかは入室順に俯瞰画面の「次の人は手を挙げてください」で決める。
// スマホの差分は案内の文言（「マーカー ID n を向けて」→「俯瞰画面の案内で手を挙げて」）と接続先のパスだけ。ゲームの仕様・UI・HUD は 08 のまま。
//   - 位置の取り方（08 との差分。08-3 と同じ）: スマホはマーカーを検出しない（createMarkerAnchor を使わない）。部屋に固定した PC の
//     Web カメラ（俯瞰画面のトラッカー。overview.html）が、手を挙げて割り当てた人物の頭の位置（welcome の trackId の番号で送る）と
//     ヨー（肩と耳の向き）を部屋（field）座標系でサーバーへ送り、サーバーが全員に配る（tracked）。スマホは自分の位置を
//     そこから受け取り、回転はジャイロ（DeviceOrientationControls）のまま。アンカー（field → ワールド）は
//     「Y 回転 φ + 並進」だけ: φ = ワールドでのカメラのヨー − トラッカーのヨー（正面を向いて肩が大きく映った =
//     品質が高いときだけ緩やかに補正 → ジャイロのヨーのドリフトを消す）、並進 = カメラ位置 − R_y(φ)・自分の位置。
//     位置は LAN の遅延ぶん遅れて届くので前回値から lerp し、見失った間は最後の位置を保持して HUD に "tracked (0.8s ago)"。
//     コート（field）は PC カメラの座標系に固定されるので、どこを向いてもコートは動かず、相手の位置は同じカメラで測られる。
//     スマホのカメラは手の検出とパススルーだけに使う（?fakecam=1 はチェッカーボードだけ描く）
...
//   - マルチマーカー（issue #30）の配置は 08 と同じく俯瞰画面から配られるが、08-8 のスマホは検出には使わない
//     （トラッカーの原点合わせの候補になる。床のマーカー等）。各マーカーの枠は配置どおりに描くので、実物と重ならなければ
//     トラッカーの原点合わせがずれている（貼りズレ か 入力ミス か 原点確定時の姿勢の誤差）
...
// アウトサイドイン（08-3・08-8 共通）固有（08 の detW / smooth / markerIntervalMs / lostMs / maxPoseError の代わり）
/** サーバーから届いた位置へ前回値から寄せる係数（1 フレームあたり。1 で補間なし） */
...
// デバッグ。?fakecam=1 はチェッカーボードだけ（スマホはマーカーを検出しないので、08 の fakeMarkers / fakeCamPos / fakeShift 等は無い。
// 位置はトラッカー（俯瞰画面の ?fakecam=1&fakeBodies=）から届く）
...
// 08 の ?gravityAlign= は無い: アンカーは常に Y 回転だけ（ワールドの上 = ジャイロの重力の上 = field の Y）なので常に水平
...
// ---- アンカー（field 座標系 → ワールド）。姿勢は updateAnchor がトラッカーの位置 + ジャイロのヨーから毎フレーム決める ----
const anchor = new THREE.Group();
anchor.visible = false;
scene.add(anchor);
const field = anchor;

// マーカーの枠（原点 + 追加マーカー。配置どおりの位置に描く。追跡中は青、見失っている間は赤）。
// 08-8 のスマホはマーカーを検出しないが、枠が実物のマーカーに重なるかで「トラッカーの原点合わせが合っているか」を目で確かめられる
...
/** サーバーの配置（config.markers）を取り込む。変わっていれば枠を作り直す */
...
// 08-8 のスマホはマーカーを検出しないので、フェイクカメラはチェッカーボード（動くバー）だけ。合成の手（?fakehands=1）はこれに重ねる
...
// ---- 自分の位置（トラッカー → サーバー → ここ）とアンカーの姿勢 ----
// サーバーの tracked から自分（selfId）の位置・ヨー・品質を取り出す。位置は毎フレーム前回値から lerp（LAN の遅延ぶん遅れて届く）。
// ヨーはジャイロ（カメラのワールド姿勢）で連続させ、品質の高い観測が来たときだけ φ（field → ワールドの Y 回転）を緩やかに寄せる
/** 自分の番号（welcome の trackId。トラッカーが手を挙げた人物に割り当てるとこの番号で位置が届く。HUD に出す） */
let myTrackId: number | null = null;
/** サーバーのトラッカーの状態（接続数・原点の確定）。welcome / tracked で更新 */
let oiStatus: TrackerStatus = { connected: 0, locked: false };
/** 直近の自分の観測（field 座標系）。一度も無ければ null */
let trackedTarget: { pos: THREE.Vector3; yaw: number; quality: number } | null = null;
/** 直近に自分の観測を受けた時刻 [ms]。一度も無ければ -Infinity */
let lastTrackedMs = -Infinity;
/** 受けた観測の数（HUD） */
let trackedCount = 0;
/** 補間中の自分の位置（field 座標系） */
...
let lastYawQuality = 0;

const camForward = new THREE.Vector3();
/** ワールド（ジャイロ）でのカメラのヨー [rad]（カメラの -Z を水平面に射影） */
function cameraYawWorld(): number {
  camForward.set(0, 0, -1).transformDirection(camera.matrixWorld);
  return yawOfForward([camForward.x, camForward.y, camForward.z]);
}

function isTracked(now: number): boolean {
...
function onTracked(status: TrackerStatus, players: TrackedPlayer[]) {
  ...
  const now = performance.now();
  trackedCount++;
  const first = trackedTarget === null;
  if (!trackedTarget) trackedTarget = { pos: new THREE.Vector3(), yaw: 0, quality: 0 };
...
const camWorldPosForAnchor = new THREE.Vector3();
let lastAnchorUpdateMs = performance.now();
/** 毎フレーム: 位置を補間し、アンカー（field → ワールド）の姿勢を「カメラ位置 − R_y(φ)・自分の位置」に置く */
function updateAnchor(now: number) {
  const dtFrames = Math.min((now - lastAnchorUpdateMs) / (1000 / 60), 4);
  lastAnchorUpdateMs = now;
  if (!trackedTarget || !phiSet) return;
  const alpha = 1 - Math.pow(1 - TRACK_SMOOTH, dtFrames);
  trackedPos.lerp(trackedTarget.pos, alpha);
  camera.getWorldPosition(camWorldPosForAnchor);
  const pose = anchorPose(anchorPhi, [camWorldPosForAnchor.x, camWorldPosForAnchor.y, camWorldPosForAnchor.z], [trackedPos.x, trackedPos.y, trackedPos.z]);
...
/** HUD の track= の行（08 の marker= と同じ位置に出す） */
function trackInfo(now: number): string {
  if (oiStatus.connected === 0) return "no-tracker";
  if (!oiStatus.locked) return "unlocked";
  if (!trackedTarget) return `waiting id=${myTrackId ?? "?"}`;
  const age = (now - lastTrackedMs) / 1000;
  const base = `id=${myTrackId ?? "?"} q=${trackedTarget.quality.toFixed(2)} yaw=${THREE.MathUtils.radToDeg(trackedTarget.yaw).toFixed(0)}deg phi=${THREE.MathUtils.radToDeg(anchorPhi).toFixed(0)}deg Δyaw=${lastPhiCorrectionDeg.toFixed(1)}deg(q${lastYawQuality.toFixed(2)}) n=${trackedCount}`;
  return isTracked(now) ? `${base} ${(age * 1000).toFixed(0)}ms` : `${base} tracked (${age.toFixed(1)}s ago)`;
}
...
function connect(name: string) {
  ...
  client = connectGame(
    ...
    },
    {
      ...
      onWelcome: (id, _role, peerIds, cfg, state, info) => {
        selfId = id;
        netStatus = "open";
        joined = true;
        posesSent = 0;
        // 再接続すると id が変わるので、トラッカーの割当（人物 → プレイヤー）も外れる。俯瞰画面の案内でもう一度手を挙げてもらう
        myTrackId = info.trackId ?? null;
        ...
        splatted.clear();
        onState(state);
        console.log(`[game] joined "${ROOM}" as ${id} color=${myColor} trackId=${myTrackId} tracker=${info.tracker.connected}/${info.tracker.locked ? "locked" : "unlocked"} (peers: ${peerIds.join(", ") || "none"})`);
      },
      onPeerJoin: (id, trackId) => {
        createPeer(id);
        console.log(`[game] peer ${id} joined (trackId ${trackId})`);
      },
      onPeerLeave: (id) => {
        removePeer(id);
...
function sendPoseIfDue(now: number) {
  if (!client || !anchor.visible) return;
  ...
  // 08 の markerIds（どのマーカーで位置合わせしているか）は付けない: 位置合わせはトラッカーの側で、俯瞰画面は tracked から表示する
...
function renderHud() {
  ...
  const text = [
    ...
    `track=${trackInfo(now)}${anchor.visible && !isTracked(now) ? " (holding last pose)" : ""} layout=${describeMarkers(fieldCfg.markers ?? [])} self=${lastSelfPos ? `(${lastSelfPos.map((v) => v.toFixed(2)).join(",")})` : "-"}`,
...
// ヘッドレス確認用（scripts/headless-08-8-pose-cam.mjs）: 自分の field 座標系での位置・向き、ピアの位置
const dbgForward = new THREE.Vector3();
const dbgQuat = new THREE.Quaternion();
(window as Window & { __poseCam?: unknown }).__poseCam = {
  /** 補間中の自分の位置（field 座標系） */
  selfPos: (): V3 => [trackedPos.x, trackedPos.y, trackedPos.z],
  /** 自分の正面（カメラの -Z）を field 座標系に直したもの */
  selfForward: (): V3 => {
    field.getWorldQuaternion(dbgQuat).invert();
    dbgForward.set(0, 0, -1).transformDirection(camera.matrixWorld).applyQuaternion(dbgQuat);
    return [dbgForward.x, dbgForward.y, dbgForward.z];
  },
  ...
  reconnect: () => {
    const name = readPlayerName();
    client?.dispose();
    // サーバーが古い接続の退室を処理してから入り直す（空いた番号を引き継げるように）
...
renderer.setAnimationLoop(() => {
  ...
  for (const f of [originFrame, ...extraFrames]) {
    f.material.color.setHex(!tracking ? 0xf28b82 : 0x8ab4f8);
  }
