// 08-8 用のマーカー印刷ページ。08-3 の markers.ts のコピーから、ゴーグルのマーカーを外したもの（08-8 はゴーグルに何も貼らない）。
// 原点（正面の壁）+ 床（トラッカーの原点合わせ用）を出す。
// SVG は検出と同じ辞書（marker-detector.ts の markerSvg）から生成するので、03 の marker-0.svg と同じ描き方。
// ?mm= で黒い正方形の一辺 [mm]、?ids= で印刷する ID の並び（既定 0,1）、?markerId= で原点の ID
...
if (idsRaw) {
  ...
  if (floor && floor.id !== ORIGIN_ID) sheets.push({ id: floor.id, title: `${floor.label}（トラッカーの原点合わせ用）— ID ${floor.id}`, up: "↑ 上（正面の壁の方向へ）" });
}
...
for (const s of sheets) {
  ...
  marker.style.setProperty("--marker-mm", String(MM));
  marker.innerHTML = markerSvg(s.id);
  const caption = document.createElement("div");
  caption.className = "caption";
  caption.innerHTML = `<strong>${s.title}</strong><small>黒い正方形 ${MM}mm ／ ARUCO_MIP_36h12 ／ 08-8 MR Splatoon（Pose カメラ）。中心の位置を原点から測って俯瞰画面に入力</small>`;
